...
> **Read the re-run first.** Everything below Method is App Atlas **v0.5.0** and is left
> exactly as it was written, because a bake-off you edit after the fact is not a
> measurement. [The re-run of 30 July](#re-run-30-july-2026--v060-on-the-same-two-subjects)
> re-measures the same two subjects against v0.6.0 and says which of the three losses
> closed.
...
# Re-run, 30 July 2026 — v0.6.0 on the same two subjects

*Different machine (macOS, not the research container), same two subjects, same
`--no-ai`, `--fresh` so nothing came out of a cache. Only App Atlas was re-run; the
competitor numbers above stand as measured on 27 July.*

## The doors, then and now

| | Fixture (A) | Real app (B) |
|---|---|---|
| App Atlas v0.5.0 | 9 ways in | 19 ways in |
| **App Atlas v0.6.0** | **23 ways in** | **39 ways in** |
| OWASP Noir (27 Jul) | 5 app + 9 synthesized | 3 app + 32 synthesized |

The new doors are the Supabase PostgREST surface — the thing Result 1 recorded Noir
finding and App Atlas missing. Both runs completed in under a second.

## Loss 1 — closed, and past where Noir stopped

Noir synthesized the REST endpoints a Supabase table exposes. App Atlas now does that
*and* reads the migrations to say whether each one is locked:

```
DELETE /rest/v1/users        | RLS · 2 policies
GET    /rest/v1/customers    | RLS · no policy, so nothing is allowed through
POST   /rest/v1/audit_log    | NO GUARD
```

That third line is the one worth having. `audit_log` is a table in the fixture with RLS
never enabled — a door with no lock on it at all — and it reads differently on the page
from `customers`, which is bolted shut, and from `users`, which has two policies. Noir
finds the door. Nobody else in this document says whether it is locked.

The confidence stays `likely` rather than `certain`, for the reason
[LANDSCAPE.md](LANDSCAPE.md) gives about dashboard-created policies: a migration is
evidence of what protects a table, not proof of everything that does.

## Loss 2 — closed, and the re-run found the same bug one size smaller

The v0.5.0 over-claim was `10 of 10 are read by the code but written down nowhere` on a
repo that documents 7 of them in `.env.local.example`. v0.6.0 prints:

```
* not in .env.example and .env.local.example — 2 of 10 are read by the code
  but written down nowhere.
```

Both files read, both named, and the two genuinely undocumented variables
(`NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY_LIVE`, `STRIPE_SECRET_KEY_LIVE`) are correct.

**But the re-run printed three stars above the words "2 of 10".** The star meant
"undocumented"; the count underneath excluded variables the hosting platform sets, and
`NEXT_PUBLIC_VERCEL_URL` is one. Nobody forgot to write that down — Vercel provides it —
so excluding it from the count was right and starring it anyway was not. A reader who
counted the marks found the tool contradicting itself inside one paragraph.

Fixed in this branch: platform variables now carry `†` and their own footnote, and a test
asserts the stars and the number agree. It is a small thing, and it is the same failure
as the original, which is why it is recorded here rather than quietly patched.

## Loss 3 — partly closed, and honest about the part that isn't

v0.5.0 headline: `12 of 17 doors reachable from the internet have no auth check`, nine of
them sign-in helpers. v0.6.0:

```
11 of 37 routes have no auth check App Atlas can see
1 more are pages or the door people sign in through, open on purpose
```

The denominator is honest now and the "public by design" class exists — but it caught
**one** door on this repo, and `signInWithEmail`, `signInWithPassword`, `signUp`,
`requestPasswordUpdate` and `SignOut` are all still counted as findings. The rule
recognises a sign-in *page*; it does not yet recognise a sign-in *server action*. The
alarm is smaller and better-worded than it was, and it is still inflated by roughly five.

## CodeBoarding, re-verified — with one correction to our sharpest claim

Installed from `main` on a clean machine (`uv sync --frozen`) and pointed at the fixture,
CodeBoarding produced exactly what it produced in the container:

```
ERROR  LLM provider not configured: No LLM provider selected. Set one of:
ANTHROPIC_API_KEY, AWS_BEARER_TOKEN_BEDROCK, CEREBRAS_API_KEY, … OLLAMA_BASE_URL,
OLLAMA_HOST, OPENAI_API_KEY, …
```

A `.codeboarding/logs/` directory, and nothing else. Result 4 holds, ten months and one
operating system later.

**The correction:** that error list includes `OLLAMA_BASE_URL` and `OLLAMA_HOST`.
CodeBoarding does not require a *paid* key — it requires a *model*, and that model may be
a free local one. "Works with no API key" therefore overstates the difference. The
accurate claim is narrower and still true:

> App Atlas needs no model at all. Every number in this document — every door, every auth
...
1. **Recommendation 3 from July is done, and it is now a lead.** "Which of your database
   tables are exposed to the internet, and which of those have no policy" is a sentence no
   competitor in either document can say.
2. **Recommendation 4 is half-done.** Finish "public by design" for auth *actions*, not
   only pages.
3. **Stop saying "no API key" and start saying "no model."** It is a smaller claim that
   survives contact with CodeBoarding's own error message.
4. **The comparison that matters next is CodeAtlas, not CodeBoarding** — see
...
For the 30 July re-run, add `--fresh` to `analyze` so nothing is served from the
incremental cache, and:

```bash
git clone --depth 1 https://github.com/CodeBoarding/CodeBoarding && cd CodeBoarding
uv sync --frozen && uv run codeboarding --local <subject>
```
