...
const OTHER_DOORS = new Set(['webhook', 'cron', 'queue', 'worker', 'cli', 'file-read', 'ipc']);
const MAX_PORTS = 20;
export function renderAtlasMarkdown(graph, options = {}) {
    ...
    out.push('');
    out.push(`> Generated by App Atlas v${options.toolVersion ?? meta.toolVersion} on ${(options.generatedAt ?? meta.generatedAt).slice(0, 10)}. Facts are derived by each language's own parser${tier ? '' : ' and type checker'}; sentences marked \`(ai)\` were generated and may be wrong. Re-run \`app-atlas export\` after the code changes.`);
    if (tier) {
        out.push('>');
    ...
    out.push('');
    out.push(`- ${n(stats.files)} files · ${n(stats.linesOfCode)} lines · ${n(stats.modules)} folders · ${n(stats.functions)} functions · ${n(stats.types)} types`);
    out.push(`- ${n(stats.endpoints)} ways in · ${n(stats.externalServices)} outside ${plural(stats.externalServices, 'service')} · ${n(stats.stores)} data ${plural(stats.stores, 'store')} · ${n(stats.envVars)} env vars`);
    if (meta.frameworks.length > 0)
        out.push(`- Built with ${meta.frameworks.join(', ')}`);
    ...
    out.push(stats.files === 0
        ? '- No files App Atlas could read, so nothing here is scored for documentation'
        : `- ${percent(stats.documentedFiles, stats.files)} of files carry a docstring App Atlas reads verbatim${drift ? ` · ${stats.staleDocs} describe code that has since changed` : ''}`);
    ...
    if (stats.unreadFiles > 0) {
        out.push(`- **App Atlas could not read ${n(stats.unreadFiles)} ${plural(stats.unreadFiles, 'file')}**, so whatever ` +
    ...
    if (insights.auth.routes.length > 0) {
        ...
        if (auth) {
            ...
            for (const caveat of auth.caveats)
                out.push(`- ${sentenceCase(caveat)}.`);
            out.push('');
        }
        out.push('| Kind | Route | Auth | Writes | Where |');
        out.push('|---|---|---|---|---|');
        for (const route of insights.auth.routes.slice(0, MAX_ROUTES)) {
            out.push(`| ${route.method ?? route.endpointKind} | ${route.route ?? route.name} | ${authCell(route)} | ${route.writes ? 'yes' : '—'} | ${route.sites[0]?.path ?? '—'} |`);
        }
    ...
    const others = graph
        ...
        .filter((node) => OTHER_DOORS.has(String(node.meta.endpointKind)));
    if (others.length > 0) {
    ...
    if (insights.stores.length + insights.services.length > 0) {
        ...
        for (const store of insights.stores) {
            ...
            if (store.catalogTables.length > 0) {
                ...
                out.push(`  - Also queries the database's own catalog (${n} ${plural(n, 'row')}: ` +
        ...
        for (const service of insights.services) {
            out.push(`- **${service.name}** (${service.category}) — ${service.callSites} call ${service.callSites === 1 ? 'site' : 'sites'}${service.evidence.length > 0 ? ` via ${service.evidence.slice(0, 2).join(', ')}` : ''}`);
        }
        out.push('');
    }
    // --- the parts ---
    ...
    if (parts.length > 0) {
        ...
        for (const part of parts) {
            ...
            const scope = Number.isFinite(covered) && covered < files
    ...
    if (shapes.length > 0) {
        ...
        for (const card of shapes) {
            const shape = card.fields.length > 0
                ? card.fields.map((field) => `${field.name}${field.optional ? '?' : ''}`).join(', ')
                : (card.aliasOf ?? '');
            out.push(`- **${card.name}** (${card.typeKind}, \`${card.path}\`)${card.usage > 0 ? ` — used in ${card.usage} places` : ''}${shape ? ` — ${shape}` : ''}`);
        }
    ...
    if (insights.env.total > 0) {
        ...
        if (insights.env.undocumented.length > 0) {
            out.push('');
            out.push(`\\* not in ${insights.env.exampleFile ?? '.env.example'} — ${insights.env.undocumented.length} of ${insights.env.total} are read by the code but written down nowhere.`);
        }
    ...
    if (start.length > 0) {
        ...
        out.push('The files that pull the most of this app together — entry points, and the modules that wire them up. Ranked by what they depend on, not by how many things touch them.');
        out.push('');
    ...
    out.push('---');
    out.push('');
    out.push('This file is generated. The full atlas — every file, function, type and edge — is in `.app-atlas/atlas.json`, and `app-atlas serve` opens the same information as a map you can click through.');
    out.push('');
    return out.join('\n');
}
/**
 ...
 * would produce.
 */
function appendChanges(out, changes, shared) {
    if (shared) {
        out.push('## What changed since the last run');
        out.push('');
        out.push('Not recorded here. This file is committed, and what changed since the last run is a comparison against ' +
    ...
    out.push('## What changed since the last run');
    out.push('');
    out.push(report.tone === 'warn' ? `**${sentenceCase(report.headline.text)}.**` : `${sentenceCase(report.headline.text)}.`);
 ...
 */
function appendPorts(out, graph) {
    const ports = graph
        .nodesOfKind('endpoint')
        .filter((node) => String(node.meta.endpointKind) === 'port');
    if (ports.length === 0)
        return;
    out.push('## Ports a deployment file publishes');
    out.push('');
    out.push('Declared in the files named below, not observed on any machine. Nothing here says the stack is running, ' +
    ...
    if (ports.length > MAX_PORTS)
        out.push(`- …and ${ports.length - MAX_PORTS} more.`);
    out.push('');
}
/**
 ...
 * page.
 */
function appendOtherDoors(out, doors) {
    const line = (door) => {
    ...
    const cli = doors.filter((door) => door.meta.endpointKind === 'cli');
    const rest = doors.filter((door) => door.meta.endpointKind !== 'cli');
    ...
    for (const door of cli) {
        const path = door.meta.sites?.[0]?.path ?? '';
        const folder = path.includes('/') ? path.slice(0, path.lastIndexOf('/')) : '';
        const list = byFolder.get(folder);
        if (list)
            list.push(door);
        else
            byFolder.set(folder, [door]);
    }
    for (const [folder, group] of [...byFolder].sort((a, b) => a[0].localeCompare(b[0]))) {
        ...
        out.push(`- **cli** ${group.length} scripts under \`${folder || 'the repo root'}\`, run one at a time — ` +
            `${group
                .slice(0, CLI_SHOWN_WHEN_FOLDED)
                .map((door) => `\`${(door.meta.sites?.[0]?.path ?? '').split('/').pop()}\``)
 ...
 * decides.
 */
function appendRetired(out, graph) {
    const retired = graph
        .allNodes()
    ...
    out.push('');
    out.push(`${retired.length} ${plural(retired.length, 'file')} ${retired.length === 1 ? 'describes itself' : 'describe themselves'} ` +
...
function appendPersonalData(out, graph) {
    ...
    out.push('');
    out.push('Matched on column **names** only — App Atlas has not read a single value, and this is not a ' +
    ...
    for (const table of report.tables.slice(0, MAX_PERSONAL_TABLES)) {
        ...
        const doors = table.doors.length > 0
            ? ` — reached by ${table.doors.slice(0, MAX_REACHING_DOORS).map((door) => `\`${door.name}\``).join(', ')}${table.doors.length > MAX_REACHING_DOORS ? ` and ${table.doors.length - MAX_REACHING_DOORS} more` : ''}`
            : '';
    ...
    if (report.ambiguousOnly.length > 0) {
        out.push('');
        out.push(`${n(report.ambiguousOnly.length)} further ${plural(report.ambiguousOnly.length, 'table', 'tables')} ` +
    ...
    if (report.unknownColumns.length > 0) {
        out.push('');
        out.push(report.unknownColumns.length === 1
 ...
 */
function appendUnimported(out, view) {
    ...
    out.push(`**${sentenceCase(view.headline ?? '')}** — out of ${n(view.considered)} weighed up. ` +
    ...
    if (view.total > view.files.length)
        out.push(`- …and ${view.total - view.files.length} more.`);
    out.push('');
...
function appendDoors(out, doors) {
    if (doors.length === 0)
        return;
    out.push('');
    ...
    if (doors.length > MAX_CHANGED_DOORS)
        out.push(`- …and ${doors.length - MAX_CHANGED_DOORS} more.`);
}
