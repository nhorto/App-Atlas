...
const SCOPE_PROPERTY = {
    scope: {
        type: 'string',
        description: 'In a monorepo, which app to ask about — its name as `app-atlas analyze` reported it. ' +
...
export const MCP_TOOLS = [
    {
        ...
        description: 'Every route into this app that has no authentication check, and the ones that are unchecked for a stated ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                includeExplained: {
                    type: 'boolean',
                    description: 'Also list the unchecked doors that have a reason — a page the browser renders for anyone, the address ' +
    ...
    {
        name: 'list_doors',
        description: 'Every way into this app, of every kind: HTTP routes, server actions, webhooks, scheduled jobs, queue workers, ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                kind: {
                    type: 'string',
                    description: 'Only doors of this kind: http-route, server-action, webhook, cron, queue, realtime, cli, file-read, ' +
                        'export, screen.',
                },
    ...
    {
        name: 'trace_error',
        description: 'Put a stack trace on the map: paste one and get back which of your files and functions each frame lands in, ' +
        ...
        inputSchema: {
            ...
            properties: {
                trace: {
                    type: 'string',
                    description: 'The error as it was given to you — the whole paste, including the message line and any log noise ' +
    ...
    {
        name: 'what_calls',
        description: 'Who reaches a function, type, file or door — the callers, the importers and the routes that expose it. Use it ' +
        ...
        inputSchema: {
            ...
            properties: {
                targets: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'The things to ask about, each an atlas node id (`func:src/lib/db.ts#connect`) or a plain name ' +
    ...
    {
        name: 'where_is',
        description: 'Find something by name and get its file, its lines, what contains it, and its description. Descriptions read ' +
        ...
        inputSchema: {
            ...
            properties: {
                queries: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'The names to look for, each a name, part of a name, or part of a path. Prefer this over "query" ' +
    ...
    },
    {
        ...
        description: 'Which files nothing else in this app imports — the abandoned drafts an agent left behind, if there are any. ' +
    ...
    {
        name: 'data_stores',
        description: 'Where this app keeps data: every database, cache, bucket and browser store, the client library it goes ' +
    ...
    {
        name: 'env_vars',
        description: 'Every environment variable this code reads, where it is read, whether the name looks like a credential, and ' +
 ...
 */
export function callMcpTool(source, name, args) {
    const found = source.resolve(readString(args, 'scope'));
    if (!found.found)
        return problem(found.because);
 ...
 */
function unguardedDoors(graph, app, apps, includeExplained) {
    ...
    if (!headline) {
        ...
        lines.push('Nothing in this project answers a URL, so there is no door a stranger can knock on and nothing here to guard.');
        if (otherDoors.length > 0) {
            const kinds = [...new Set(otherDoors.map((node) => endpointMeta(node).endpointKind))].sort();
            lines.push(`It does have ${otherDoors.length} ${plural(otherDoors.length, 'way', 'ways')} in of other kinds ` +
    ...
    const explained = insights.auth.routes.filter((route) => route.open && route.open.kind !== 'worth-a-look');
    lines.push(`${sentenceCase(headline.headline)}.`);
    for (const caveat of headline.caveats)
        lines.push(`- ${sentenceCase(caveat)}.`);
    ...
    if (explained.length > 0) {
        ...
        if (!includeExplained)
            lines.push('  Pass includeExplained to see them listed.');
    }
    if (insights.auth.unread.length > 0) {
        lines.push('');
        lines.push(`${insights.auth.unread.length} ${plural(insights.auth.unread.length, 'file', 'files')} could not be parsed at ` +
            'all, so whatever they declare is missing from every number above:');
        for (const file of insights.auth.unread.slice(0, 10))
            lines.push(`  ${file.path} — ${file.because}`);
    }
    return answer(lines, provenance(app, apps, graph), {
        ...envelope(app, graph),
...
function listDoors(graph, app, apps, kind, limit) {
    const all = graph
        .nodesOfKind('endpoint')
        .filter((node) => !NOT_A_DOOR.has(endpointMeta(node).endpointKind))
    ...
    for (const node of shown) {
        ...
        if (meta.schedule)
            parts.push(`runs ${meta.schedule}`);
        if (meta.writes)
            parts.push('writes data');
    ...
    if (all.length > shown.length)
        lines.push(`  …and ${all.length - shown.length} more — raise limit to see them.`);
    return answer(lines, provenance(app, apps, graph), {
        ...envelope(app, graph),
        total: all.length,
 ...
 */
function traceErrorTool(graph, app, apps, pasted, limit) {
    if (!pasted.trim())
        return problem('trace_error needs a "trace" — paste the error as you received it.');
    const result = traceError(graph, pasted, bundleMaps(graph.meta.root), installedPackages(graph.meta.root));
    const lines = [];
    if (result.parsedNothing) {
        lines.push('Nothing in that paste looked like a stack frame, so there is no file or line to start from. This tool needs ' +
    ...
    for (const found of result.frames) {
        ...
        if (found.nodeId) {
            ...
            lines.push(`  ${where}  →  ${found.nodeKind} ${found.nodeName}  ·  ${found.path}:${found.sourceLine}${mapped}${drift}`);
        }
        else {
    ...
    if (result.needsSourceMap) {
        lines.push('Some of those frames are inside build output that no source map in this project places, so their line ' +
    ...
    lines.push('');
    if (!result.origin) {
        lines.push('None of those frames is code in this project, so there is no frame here to trace back from. The failure ' +
        ...
        if (into && into.importers.length > 0) {
            ...
            if (rest > 0) {
                lines.push(`  …and ${rest} more ${plural(rest, 'file', 'files')}. The ones above are those the most ways in can ` +
                    'reach, which is an ordering and not a ranking of suspects.');
            }
        }
        else if (into && into.total > 0) {
            lines.push('');
            lines.push(`The innermost frame is inside ${into.packageName}, and ${into.total} files here import ` +
            ...
            lines.push('');
            lines.push(`The innermost frame is inside ${into.packageName}. Nothing here imports it and no dependency of this ` +
    ...
    lines.push(`Deepest frame in your own code: ${result.origin.nodeName} — ${result.origin.path}:${result.origin.sourceLine ?? result.origin.frame.line}`);
    lines.push('');
    const shown = result.doors.slice(0, limit);
    if (result.doors.length === 0) {
        lines.push('No way into the app reaches that code by any reference this analysis can follow. That is "none found", not ' +
            '"none exists" — code called through a dynamic lookup, a string name or reflection leaves no edge behind.');
    }
    else {
        lines.push(`${result.doors.length} ${plural(result.doors.length, 'way in can', 'ways in can')} reach it. ` +
            'Any of them could be the one that ran; the code does not say which.');
        for (const reach of shown) {
            lines.push(`  ${reach.door.method ?? reach.door.endpointKind} ${reach.door.route ?? reach.door.name}  ·  ` +
...
function whyUnplaced(reason, candidates) {
    switch (reason) {
        case 'dependency':
 ...
 */
function batched(app, apps, graph, sections, dropped, footer = []) {
    const lines = [];
    if (dropped > 0) {
        lines.push(`Answering the first ${MAX_TARGETS}; ${dropped} more ${plural(dropped, 'was', 'were')} not looked at. ` +
...
function whatCalls(graph, app, apps, targets, dropped, limit) {
    if (targets.length === 0) {
...
function whatCallsOne(graph, app, target, limit) {
    ...
    if ('candidates' in resolved) {
        ...
        for (const node of resolved.candidates)
            lines.push(`  ${node.id}  ${node.kind}  ${placeOf(node)}`);
        return {
            target,
            lines,
    ...
    if (incoming.length === 0) {
        lines.push('Nothing in this atlas reaches it. That means none was found, not that none exists: a call made through a ' +
        ...
        for (const edge of shown) {
            ...
            if (!other)
                continue;
            lines.push(`  ${edge.kind}  ${other.kind} ${other.name}  ·  ${edge.confidence}  ·  ${placeOf(other)}`);
        }
...
function whereIs(graph, app, apps, queries, dropped, limit) {
    if (queries.length === 0)
        return problem('where_is needs a "query", or several in "queries".');
...
function whereIsOne(graph, app, query, limit) {
    ...
    for (const node of matches) {
        ...
        if (node.summary)
            lines.push(`      ${oneLine(node.summary)}${node.summarySource === 'ai' ? ' (ai)' : ''}`);
    }
 ...
 */
function unimportedFiles(graph, app, apps) {
    ...
    if (view.total === 0) {
        ...
        lines.push(`${sentenceCase(view.headline ?? '')}, out of ${view.considered} weighed up. No import, no door, ` +
        ...
        if (view.total > view.files.length)
            lines.push(`  ...and ${view.total - view.files.length} more`);
    }
    lines.push('');
    for (const caveat of view.caveats)
        lines.push(`- ${sentenceCase(caveat)}.`);
    return answer(lines, provenance(app, apps, graph), {
        ...envelope(app, graph),
        answered: true,
...
function dataStores(graph, app, apps) {
    const insights = buildInsights(graph);
    const lines = [];
    ...
    if (insights.stores.length > 0) {
        ...
        for (const store of insights.stores) {
            ...
            lines.push(`  ${store.name}  ·  ${store.storeKind} via ${store.client}  ·  ` +
                `${store.reads} ${plural(store.reads, 'read', 'reads')}, ${store.writes} ${plural(store.writes, 'write', 'writes')}${tables}`);
        }
    }
    if (insights.tables.total > 0) {
        lines.push('');
        lines.push(`${insights.tables.total} ${plural(insights.tables.total, 'table', 'tables')}, and what protects each one's rows:`);
        ...
        if (insights.tables.unknown > 0) {
            lines.push('');
            lines.push(`${insights.tables.unknown} of those ${plural(insights.tables.unknown, 'tables has', 'tables have')} nothing ` +
    ...
    if (personal.tables.length > 0) {
        lines.push('');
        lines.push(`${personal.tables.length} of those ${plural(personal.tables.length, 'tables has', 'tables have')} column ` +
...
function envVars(graph, app, apps, undocumentedOnly) {
    ...
    lines.push(`${env.total} environment ${plural(env.total, 'variable', 'variables')} are read by this code. ` +
 ...
 */
function provenance(app, apps, graph) {
    const lines = [];
    lines.push(`Source: the atlas of "${app.name}" in ${app.dir}, analysed ${graph.meta.generatedAt} by App Atlas ` +
    ...
    if (tier)
        lines.push(tier.sentence);
    if (apps.length > 1) {
        const others = apps.filter((other) => other.dir !== app.dir).map((other) => other.id || other.name);
        lines.push(`This project holds ${apps.length} apps and this answer is only about "${app.name}". The others are ` +
 ...
 */
function staleness(app, graph) {
    const analyzed = graph.meta.vcs?.commit;
    if (!analyzed)
        return null;
    const current = headCommit(app.dir);
    if (!current || current === analyzed)
        return null;
    return (`This code has moved since it was analysed: the atlas describes ${shortCommit(analyzed)} and the working tree ` +
...
function callerFact(edge, other) {
    if (!other)
        return null;
    return {
        edgeKind: edge.kind,
...
// Rendering
// ---------------------------------------------------------------------------
function answer(lines, footer, structured) {
...
function routeLine(route) {
    ...
    if (route.writes)
        parts.push('writes data');
    if (route.open?.because)
        parts.push(route.open.because);
    parts.push(siteOf(route.sites, null));
    return parts.filter(Boolean).join('  ·  ');
}
function doorName(node, meta) {
    if (!meta.route)
        return node.name;
...
function reasonPhrase(kind, count) {
    switch (kind) {
        case 'page':
...
function rlsPhrase(rls, declared) {
    if (!rls) {
        return declared
    ...
    if (!rls.enabled)
        return 'row security off';
    if (rls.policyCount === 0)
        return 'row security on with no policy — every request is denied';
...
function resolveTarget(graph, target) {
    ...
    if (exact.length === 1)
        return { node: exact[0] };
    if (exact.length > 1)
        return { candidates: exact.slice(0, 20) };
    return { candidates: graph.search(target, 10) };
}
function endpointMeta(node) {
    return node.meta;
}
 ...
 */
function readTargets(args, singular, plural) {
    const raw = [];
    if (Array.isArray(args[plural]))
        raw.push(...args[plural]);
    if (typeof args[singular] === 'string')
        raw.push(args[singular]);
    const seen = new Set();
