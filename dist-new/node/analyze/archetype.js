...
export function classifyArchetype({ project, nodes }) {
    ...
    const noUrl = unreadFrameworks.length > 0
    ...
    if (doors.network > 0 || doors.screen > 0) {
        ...
        if (wantsEyes) {
            if (doors.screen > 0)
                because.push(plural(doors.screen, 'screen'));
            if (doors.network > 0)
            ...
            // and because the sentence it replaces said the opposite outright.
                ...
                because.push(plural(templates, 'page it renders', 'pages it renders'));
            return verdict('web-app', 'An app with a front end', because);
        }
        because.push(plural(doors.network, 'way in over the network', 'ways in over the network'));
        if (project.frameworks.length > 0)
            because.push(project.frameworks.slice(0, 3).join(', '));
    ...
    if (analysis.length > 0) {
        ...
        if (doors.declaredCli > 0)
            because.push(plural(doors.declaredCli, 'command-line entry point'));
    ...
    if (doors.declaredCli > 0 || bin.length > 0 || executables.length > 0 || crates.length > 0) {
        if (doors.declaredCli > 0)
            because.push(plural(doors.declaredCli, 'command-line entry point'));
        if (bin.length > 0)
            because.push(`${plural(bin.length, 'command')} in package.json`);
        if (executables.length > 0)
            because.push('a .NET project that builds an executable');
        if (crates.length > 0) {
            because.push(crates.length === 1
    ...
    if (exported > 0) {
        ...
        if (doors.runnableFiles > 0)
            because.push(plural(doors.runnableFiles, 'file you can also run directly', 'files you can also run directly'));
    ...
    if (project.files.length > 0)
        because.push(plural(project.files.length, 'source file'));
...
function isTestSuite(project) {
    ...
    if (!devDeps.some((name) => TEST_RUNNERS.includes(name)))
        return false;
    ...
    if (manifest.exports !== undefined || manifest.bin !== undefined || manifest.types !== undefined)
        return false;
    const main = typeof manifest.main === 'string' ? manifest.main.replace(/^\.\//, '') : null;
    if (main && project.files.some((file) => file.relPath === main))
        return false;
    return true;
}
function countDoors(nodes) {
    const counts = { network: 0, screen: 0, declaredCli: 0, runnableFiles: 0, scheduled: 0 };
    for (const node of nodes) {
        if (node.kind !== 'endpoint')
            continue;
        const meta = node.meta;
        const kind = meta.endpointKind;
        if (NETWORK_DOORS.has(kind))
            counts.network++;
        else if (kind === 'screen')
            counts.screen++;
        else if (kind === 'cli') {
            if (DECLARED_CLI.has(meta.framework))
                counts.declaredCli++;
            else
                counts.runnableFiles++;
        }
        else if (kind === 'cron' || kind === 'queue' || kind === 'worker')
            counts.scheduled++;
    }
    return counts;
}
 ...
 */
function readsDatasets(nodes, project) {
    ...
    if (project.signals.declaresAPackage)
        return [];
    const because = [];
    const notebooks = project.files.filter((file) => file.relPath.endsWith('.ipynb')).length;
    if (notebooks > 0)
        because.push(plural(notebooks, 'notebook'));
    const clients = new Set();
    for (const node of nodes) {
        if (node.kind !== 'store')
            continue;
        const meta = node.meta;
        if (typeof meta.client !== 'string' || !ANALYSIS_CLIENTS.has(meta.client))
            continue;
        if (typeof meta.reads === 'number' && meta.reads > 0)
            clients.add(meta.client);
    }
    if (clients.size > 0)
        because.push(`reads data files with ${[...clients].sort().join(', ')}`);
 ...
 */
function countExports(nodes) {
    let total = 0;
    for (const node of nodes) {
        if (node.kind !== 'file')
            continue;
        const names = node.meta.exportedNames;
        if (names && names.length > 0)
            total++;
    }
    return total;
}
function readBin(pkg) {
    const bin = pkg?.bin;
...
function verdict(archetype, label, because) {
