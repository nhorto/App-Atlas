...
export function analyzeTypeScript(ctx) {
    ...
    const edges = new Map();
    const boundaries = [];
    const registered = { byPosition: new Map(), names: new Set() };
    /**
    ...
    for (const ref of files)
        pathToRel.set(normPath(ref.absPath), ref.relPath);
    ...
    for (const ref of files) {
        ...
        for (const [pos, id] of slice.positions)
            registered.byPosition.set(`${posKey}|${pos}`, id);
        for (const node of slice.nodes) {
            if (node.kind !== 'file')
                registered.names.add(node.name);
        }
        reused++;
    }
    ...
    for (const bucket of buckets) {
        const declared = { positions: [], names: [] };
        try {
            bucket.fileNode = extractFile(bucket.ref, bucket.sf, bucket.nodes, declared);
        }
        catch (err) {
            warnings.push(`Failed to analyze ${bucket.ref.relPath}: ${err.message}`);
        }
        bucket.positions = declared.positions;
        const posKey = normPath(bucket.ref.absPath);
        for (const [pos, id] of declared.positions)
            registered.byPosition.set(`${posKey}|${pos}`, id);
        for (const name of declared.names)
            registered.names.add(name);
        if (++done % 25 === 0)
            ctx.onProgress?.('Reading files', done, buckets.length);
    }
    if (buckets.length > 0)
        ctx.onProgress?.('Reading files', buckets.length, buckets.length);
        ...
        try {
            ...
            if (bucket.fileNode) {
                ...
                if (unresolved.length > 0)
                    bucket.fileNode.meta.unresolvedImports = unresolved;
            }
        }
        catch (err) {
            warnings.push(`Failed to read imports in ${bucket.ref.relPath}: ${err.message}`);
        }
    }
    ...
    if (options.followReferences) {
        ...
        for (const bucket of buckets) {
            try {
                ...
                warnings.push(`Failed to trace references in ${bucket.ref.relPath}: ${err.message}`);
            }
            if (++done % 25 === 0)
                ctx.onProgress?.('Tracing references', done, buckets.length);
        }
        if (buckets.length > 0)
            ctx.onProgress?.('Tracing references', buckets.length, buckets.length);
    ...
    if (options.detectBoundaries) {
        ...
        for (const bucket of buckets) {
            ...
            try {
                bucket.boundaries.push(...detectBoundaries({
                    ref: bucket.ref,
                    sf: bucket.sf,
                    fileId,
                    project,
                ...
                warnings.push(`Failed to read boundaries in ${bucket.ref.relPath}: ${err.message}`);
            }
            if (++done % 25 === 0)
                ctx.onProgress?.('Finding the boundaries', done, buckets.length);
        }
        if (buckets.length > 0)
            ctx.onProgress?.('Finding the boundaries', buckets.length, buckets.length);
 ...
 * got to look".
 */
function unreadFileNode(ref, because) {
    return {
...
function extractFile(ref, sf, nodes, declared) {
    ...
    let functionCount = 0;
    let typeCount = 0;
    const register = (id, ...positions) => {
        for (const pos of positions)
            declared.positions.push([pos, id]);
    };
    // --- functions ---
    ...
    for (const decl of sf.getVariableDeclarations()) {
        ...
        if (sawFunction && isExported)
            exportedNames.push(objName);
    }
...
function interfaceNode(id, name, fileId, ref, iface) {
...
function classNode(id, name, fileId, ref, cls) {
    ...
    if (base)
        extendsList.push(base.getText());
    for (const impl of cls.getImplements())
        extendsList.push(impl.getText());
    return typeNode({
        id,
        name,
        fileId,
...
function typeAliasNode(id, name, fileId, ref, alias) {
...
function enumNode(id, name, fileId, ref, decl) {
    const fields = decl.getMembers().map((m) => ({
        name: m.getName(),
 ...
 */
function extractImports(ref, sf, pathToRel, edges) {
    const fromId = makeFileId(ref.relPath);
    const external = new Set();
    const unresolved = new Set();
    const record = (specifier, targetFile, symbols) => {
        if (targetFile) {
    ...
    for (const imp of sf.getImportDeclarations()) {
        ...
        if (def)
            symbols.push(def.getText());
        const ns = imp.getNamespaceImport();
        if (ns)
            symbols.push(`* as ${ns.getText()}`);
 ...
 * contribute an edge.
 */
function requireCalls(sf) {
    if (!sf.getFullText().includes('require('))
        return [];
    const found = [];
 ...
 */
function resolveRelative(fromAbs, specifier, pathToRel) {
 ...
 */
function extractReferences(ref, sf, registered, edges) {
    const fileId = makeFileId(ref.relPath);
    const posKey = normPath(ref.absPath);
 ...
 */
function enclosingNodeId(node, posKey, registered, fallback) {
    let field = null;
    let previous = node;
    ...
    while (current) {
        ...
        if (isFieldLike(current) && current.getTypeNode() === previous)
 ...
 */
function declaredNodeId(node, posKey, registered, fallback) {
    let current = node;
    while (current) {
        const hit = registered.byPosition.get(`${posKey}|${current.getStart()}`);
        if (hit)
...
function addEdge(edges, input) {
    ...
    if (existing) {
        ...
        for (const key of ['symbols', 'fields']) {
            const added = input.meta[key] ?? [];
            if (added.length === 0)
                continue;
            const already = existing.meta[key] ?? [];
 ...
 */
function mergeEdges(target, incoming) {
 ...
 */
function typeTextOf(written, infer, at) {
    if (written && written.trim())
        return truncate(written.trim(), MAX_TYPE_TEXT);
    try {
 ...
 */
export function extractFileDoc(text) {
