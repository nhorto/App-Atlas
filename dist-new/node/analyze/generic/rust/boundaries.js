...
export function detectRustBoundaries(input) {
    ...
    if (signals.cargoPackages.has('tauri'))
        detectCommands(input, findings);
    ...
    if (signals.cargoPackages.has('rocket'))
        detectRocketBoundaries(input, findings);
    if (signals.cargoPackages.has('sqlx'))
        detectSqlx(file, findings, at);
 ...
 * can reach.
 */
function detectCommands(input, findings) {
    ...
    const importsCommand = input.file.imports.some((imp) => (imp.module === 'tauri' || imp.module.startsWith('tauri.')) && imp.local === 'command' && !imp.alias);
    const isCommand = (attr) => {
    ...
    for (const def of input.file.defs) {
        if (def.kind !== 'function')
            continue;
        if (!def.decorators.some(isCommand))
            continue;
        findings.push({
            type: 'endpoint',
            endpointKind: 'ipc',
 ...
 * writes.
 */
function detectSqlx(file, findings, at) {
    for (const call of file.calls) {
        if (!SQLX_QUERY.test(call.callee))
            continue;
 ...
 */
function detectEnv(file, findings, at) {
    for (const call of file.calls) {
