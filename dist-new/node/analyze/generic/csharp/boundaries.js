...
export function detectCSharpBoundaries(input) {
    ...
    if (isDotnetHost(input))
        detectHostedServices(input, findings, at);
 ...
 */
function isAspNet(input) {
    const { signals, file } = input;
    if (signals.dotnetSdks?.has('Microsoft.NET.Sdk.Web'))
        return true;
    for (const id of signals.dotnetPackages ?? []) {
        if (id.startsWith('Microsoft.AspNetCore'))
            return true;
    }
...
function attributesByScope(file) {
    const out = new Map();
    for (const call of file.calls) {
        ...
        if (call.receiver || !ATTRIBUTE_NAMES.has(call.callee))
            continue;
 ...
 */
function detectControllers(input, findings, at) {
    const { file } = input;
    const attributes = attributesByScope(file);
    for (const def of file.defs) {
        if (def.kind !== 'type' || !isController(def, attributes))
            continue;
        ...
        for (const method of file.defs) {
            if (method.kind !== 'function' || method.owner !== def.name)
                continue;
 ...
 */
function isController(def, attributes) {
    const own = attributes.get(def.name) ?? [];
    if (own.some((call) => call.callee === 'ApiController' || call.callee === 'Route'))
        return true;
    if (!/Controller$/.test(def.name))
        return false;
    for (const [scope, calls] of attributes) {
        if (!scope.startsWith(`${def.name}.`))
            continue;
        if (calls.some((call) => VERB_ATTRIBUTES[call.callee]))
            return true;
    }
    return false;
}
...
function joinRoute(prefix, template) {
    if (template.startsWith('/'))
        return normalizeRoute(template);
    if (template.startsWith('~/'))
        return normalizeRoute(template.slice(1));
...
function authorizeGuard(file, call, where) {
    return {
 ...
 */
function detectMinimalApis(input, findings, at) {
    const { file } = input;
...
function verbOfMapMethods(call) {
    const verbs = call.args.filter((arg) => arg.t === 'str').slice(1);
 ...
 */
function groupPrefixes(file) {
    const direct = new Map();
    for (const call of file.calls) {
        if (bareMethod(call) !== 'MapGroup')
            continue;
...
function groupFilters(file) {
    ...
    for (const call of file.calls) {
        ...
        if (!bound)
            continue;
        const chain = chainedOnto(file, call);
        if (chain.guards.length > 0 || chain.filters.length > 0)
            out.set(bound.name, chain);
    }
    return out;
}
/**
 ...
 */
function attributedInside(file, call) {
    const out = [];
    for (const other of file.calls) {
        if (other === call || other.receiver)
            continue;
        ...
        if (other.callee === ALLOW_ANONYMOUS)
            return [];
        if (other.callee === AUTHORIZE)
            out.push(authorizeGuard(file, other, 'the handler'));
    }
    return out;
}
function chainedOnto(file, call) {
    const out = { guards: [], filters: [] };
    for (const other of file.calls) {
        if (other === call)
            continue;
        if (other.startIndex > call.startIndex || other.endIndex < call.endIndex)
        ...
        if (method === ALLOW_ANONYMOUS_CALL)
            return { guards: [], filters: [] };
        ...
        if (!method || NOT_A_FILTER.has(method) || MAP_METHODS[method] || HUB_MAPS.has(method))
            continue;
        if (!out.filters.includes(method))
            out.filters.push(method);
    }
    return out;
}
/**
 ...
 */
function detectCheckers(input, findings) {
    const { file } = input;
    for (const def of file.defs) {
        if (def.kind !== 'function')
            continue;
        const names = [...def.uses, ...def.qualifiedUses];
        const rejects = names.some((name) => REJECTION_STATUS.test(name)) ||
            names.some((name) => REJECTION_CALL.test(name)) ||
            file.calls.some((call) => call.scope === (def.owner ? `${def.owner}.${def.name}` : def.name) &&
...
function detectSignInCalls(input, findings, at) {
    const { file } = input;
    for (const call of file.calls) {
        const method = bareMethod(call);
        ...
        if (!what || !call.receiver || !call.scope)
            continue;
        findings.push({
            type: 'sign-in-call',
 ...
 */
function isDotnetHost(input) {
    if (isAspNet(input))
        return true;
    if (input.signals.dotnetSdks?.has('Microsoft.NET.Sdk.Worker'))
        return true;
    for (const id of input.signals.dotnetPackages ?? []) {
        if (id.startsWith('Microsoft.Extensions.Hosting'))
            return true;
    }
 ...
 */
function detectHostedServices(input, findings, at) {
    const { file } = input;
    const worker = (className, schedule, site) => {
        findings.push({
            type: 'endpoint',
            endpointKind: 'worker',
    ...
    for (const def of file.defs) {
        if (def.kind !== 'type' || !def.bases.some((base) => HOSTED_BASES.has(base)))
            continue;
    ...
    for (const call of file.calls) {
        if (bareMethod(call) !== 'AddHostedService')
            continue;
...
function workerSchedule(file, def) {
    for (const timer of file.calls) {
        if (timer.startIndex < def.startIndex || timer.endIndex > def.endIndex)
            continue;
        const callee = timer.callee;
        if (callee !== 'PeriodicTimer' && !/(^|\.)Task\.Delay$/.test(callee))
            continue;
        // The TimeSpan call is an argument, so its range sits inside the timer's.
        for (const span of file.calls) {
            if (span.startIndex < timer.startIndex || span.endIndex > timer.endIndex || span === timer)
                continue;
            const unit = TIMESPAN_UNITS[bareMethod(span)];
            if (!unit || !/(^|\.)TimeSpan\.\w+$/.test(span.callee))
                continue;
            const amount = span.args.find((arg) => arg.t === 'num');
            if (!amount || amount.t !== 'num')
                continue;
            if (unit === 'ms')
                return `every ${amount.v} ms`;
 ...
 */
function detectStores(input, findings, at) {
    const { file } = input;
    // --- the declared tables -------------------------------------------------
    ...
    const hasDapper = file.imports.some((imp) => imp.module === 'Dapper') ||
    ...
    const sqlFrom = (text, line, scope, snippet, method) => {
    ...
    for (const binding of file.bindings) {
        if (!/(^|\.)CommandText$/.test(binding.name) || !binding.arg)
            continue;
 ...
 */
function tableOfReceiver(receiver, tables) {
    if (!receiver)
        return null;
 ...
 */
function detectOutbound(input, findings, at) {
    const { file } = input;
    ...
    for (const call of file.calls) {
        ...
        if (writes !== undefined) {
            ...
            if (url)
                report(url, call, writes);
            continue;
        }
        if (method === 'Uri') {
            const url = literalUrl(call.args, constants);
            if (url)
                report(url, call, false);
        }
    }
}
/** The first argument that is, or resolves to, an absolute URL. */
function literalUrl(args, constants) {
    for (const arg of args) {
        if (arg.t === 'str' && /^https?:\/\//i.test(arg.v))
            return arg.v;
        if (arg.t === 'name') {
...
function detectEnv(input, findings, at) {
    for (const call of input.file.calls) {
        ...
        const isConfigRead = CONFIG_READS.has(method) ||
