 ...
 */
const PLUGINS = [typescriptPlugin, pythonPlugin, ...genericPlugins];
export async function analyzeProject(rootDir, options = {}) {
    ...
    if (cache) {
        try {
            ...
            warnings.push(`Could not save the analysis cache: ${err.message}`);
        }
        finally {
            cache.close();
        }
    ...
    const treeFiles = project.files.map((f) => ({ relPath: f.relPath, zone: f.zone }));
    if (schema.filePath)
        treeFiles.push({ relPath: schema.filePath, zone: 'data' });
    for (const relPath of sqlSchema.filePaths)
        treeFiles.push({ relPath, zone: 'data' });
    for (const relPath of markup.filePaths)
        treeFiles.push({ relPath, zone: 'ui' });
 ...
 */
function joinOrmModelsToTables(nodes) {
    ...
    for (const node of nodes) {
        ...
        if (meta.typeKind !== 'class' || !(meta.fields?.length))
            continue;
    ...
    for (const node of candidates) {
        const tableName = node.meta.tableName;
        if (!tableName)
            continue;
        const key = tableName.toLowerCase();
        const held = byTable.get(key);
        if (!held || fieldCount(node) > fieldCount(held))
    ...
    for (const node of candidates) {
        if (node.meta.tableName)
            continue;
        // Fills only the names no declaring model answered to, and never overwrites one.
        if (!models.has(node.name.toLowerCase()))
            models.set(node.name.toLowerCase(), node);
    }
    if (models.size === 0)
        return;
    for (const node of nodes) {
        ...
        if (meta.typeKind !== 'table' || !meta.observed || (meta.fields?.length ?? 0) > 0)
            continue;
...
function fieldCount(node) {
    return (node.meta.fields ?? []).length;
}
function stampSignInCalls(findings, edges, byId) {
    const byHandler = new Map();
    for (const finding of findings) {
        if (finding.type !== 'sign-in-call')
            continue;
        if (byId.get(finding.nodeId)?.kind !== 'function')
            continue;
    ...
    for (const edge of edges) {
        if (edge.kind !== 'references')
            continue;
        if (byId.get(edge.fromId)?.kind !== 'function' || byId.get(edge.toId)?.kind !== 'function')
            continue;
        const list = refs.get(edge.fromId);
        if (list)
            list.push(edge.toId);
        else
            refs.set(edge.fromId, [edge.toId]);
    }
    const reaches = (handlerId) => {
 ...
 */
function assignModuleZones(nodes) {
    ...
    const resolve = (id, seen) => {
        ...
        for (const childId of childIds.get(id) ?? [])
            zones.push(...resolve(childId, seen));
        if (node.kind === 'module')
            node.zone = dominantZone(zones);
        return zones;
    };
    for (const node of nodes) {
        if (node.kind === 'app')
    ...
    for (const node of nodes) {
        ...
        switch (node.kind) {
            case 'file':
                ...
                if (node.meta.generated)
                    generatedFiles++;
                if (node.summarySource === 'docs')
                    documentedFiles++;
                else if (node.summarySource === 'ai')
                    aiFiles++;
                break;
            case 'function':
                functions++;
