...
export const outboundDetector = {
    id: 'outbound',
    enabled: () => true,
    visit(node, ctx) {
        if (Node.isNewExpression(node))
            return sdkConstruction(node, ctx);
        if (Node.isFunctionDeclaration(node) || Node.isVariableDeclaration(node))
            return urlSink(node, ctx);
        if (!Node.isCallExpression(node))
            return;
        const dotted = dottedName(node.getExpression());
        if (!dotted)
            return;
        if (httpCall(node, dotted, ctx))
            return;
        if (urlThroughHelper(node, dotted, ctx))
            return;
...
// Literal URLs
// ---------------------------------------------------------------------------
function httpCall(call, dotted, ctx) {
    const parts = dotted.split('.');
    const root = parts[0];
    const last = parts[parts.length - 1];
    const isClient = HTTP_CLIENTS.has(root) &&
...
function report(url, call, ctx, writes) {
...
function urlSink(node, ctx) {
    ...
    for (const call of fn.getDescendantsOfKind(SyntaxKind.CallExpression)) {
        ...
        const parts = dotted.split('.');
        const last = parts[parts.length - 1];
        if (!HTTP_CLIENTS.has(parts[0]) || !(parts.length === 1 || HTTP_VERBS.has(last) || last === 'fetch'))
            continue;
 ...
 */
function urlThroughHelper(call, dotted, ctx) {
 ...
 */
function sdkConstruction(node, ctx) {
    const name = dottedName(node.getExpression());
    if (!name)
        return;
    const binding = ctx.imports.get(name.split('.')[0]);
    if (!binding?.external || HANDLED_AS_STORES.has(binding.module))
        return;
...
function exportedNameOf(node) {
    ...
    while (current) {
        ...
        if (Node.isExportAssignment(current))
            return 'default';
        ...
        if (Node.isFunctionDeclaration(current) || Node.isBlock(current))
            return null;
...
function sdkCall(call, dotted, ctx) {
    ...
    if (HANDLED_AS_STORES.has(module))
        return;
 ...
 */
function wrapperCall(call, dotted, root, ctx) {
 ...
 */
export function sendsData(dotted, category) {
    const last = dotted.split('.').pop() ?? '';
    if (/^(send|create|post|upload|track|capture|identify|charge|publish|emit|log)/i.test(last))
        return true;
