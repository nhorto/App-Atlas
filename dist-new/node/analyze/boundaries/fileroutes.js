 ...
 */
export const svelteRoutesDetector = {
    id: 'sveltekit-routes',
    ...
    fileScan(ctx) {
        ...
        if (base === '+server')
            svelteEndpoints(ctx, route);
        else if (base === '+page.server' || base === '+page')
            sveltePage(ctx, route, base === '+page.server');
    },
};
/** `+server.ts` exporting GET and POST is two doors at one address, not one. */
function svelteEndpoints(ctx, route) {
...
function sveltePage(ctx, route, isServer) {
    const exported = exportedDeclarations(ctx.sf);
    ...
    if (isServer)
        svelteActions(ctx, route, exported.get('actions'));
}
/**
 ...
 */
function svelteActions(ctx, route, decl) {
 ...
 */
export const svelteHooksDetector = {
    id: 'sveltekit-hooks',
...
function svelteSegment(segment) {
    if (segment === '')
        return null;
 ...
 */
export const remixRoutesDetector = {
    id: 'remix-routes',
 ...
 */
function remixRoutePath(id) {
    const raw = id.split('/').flatMap(flatSegments);
    const last = raw[raw.length - 1] ?? '';
    if (last !== '_index' && last.startsWith('_'))
        return null;
    const segments = raw.map(remixSegment).filter((segment) => segment !== null);
    return `/${segments.join('/')}`.replace(/\/{2,}/g, '/').replace(/(.)\/$/, '$1') || '/';
}
/**
 ...
 */
function runsInTheBrowserToo(ctx) {
    const dir = ctx.signals.svelteKitRoutesDir;
    if (!dir || !isUnder(ctx.ref.relPath, dir))
        return false;
...
function refusalName(node, ctx) {
    ...
    if (!REFUSAL_CALLS.has(callee))
        return null;
    if (!fromTheFramework(dotted.split('.')[0], ctx))
        return null;
...
function hasRefusalStatus(node) {
    if (Node.isNumericLiteral(node))
        return REFUSAL_STATUS.has(node.getLiteralValue());
...
function fromTheFramework(root, ctx) {
        ...
        if (Node.isIfStatement(current) && current.getElseStatement() !== previous) {
            ...
            current.getExpression().forEachDescendant((child) => {
                if (!Node.isCallExpression(child))
                    return;
                if (!dottedName(child.getExpression())?.endsWith('.pathname.startsWith'))
                    return;
