...
import { argAt, dottedName, functionArg, hasDirective, isRouterCallee, literalString, looksLikeRouter as isRouter, objectProp, permitsEverything, } from './ast.js';
...
// ---------------------------------------------------------------------------
export const nextRoutesDetector = {
    id: 'next-routes',
    ...
    fileScan(ctx) {
        ...
        if (appDir && isUnder(relPath, appDir)) {
            ...
            if (base === 'route')
                return appRouteHandlers(ctx, rest);
            if (base === 'page')
                return appPage(ctx, rest);
        }
...
function appRouteHandlers(ctx, rest) {
...
function pagesRouterFile(ctx, rest) {
    const base = fileBase(rest);
    if (base.startsWith('_') || base === 'middleware')
        return;
...
function serverActions(ctx) {
    ...
    for (const decl of ctx.sf.getVariableDeclarations()) {
        ...
        if (!init || !(Node.isArrowFunction(init) || Node.isFunctionExpression(init)))
            continue;
        if (!decl.getVariableStatement()?.isExported())
            continue;
        if (!fileLevel && !hasDirective(init, 'use server'))
            continue;
        ctx.emit({
            type: 'endpoint',
            endpointKind: 'server-action',
...
export const nodeRoutesDetector = {
    id: 'node-routes',
    ...
    fileScan(ctx) {
        ...
        for (const [name, local] of ctx.locals) {
            if (!isRouterCallee(local.callee))
                continue;
            ctx.emit({
                type: 'router-build',
 ...
 */
export const routeHelperDetector = {
    id: 'route-helpers',
    enabled: () => true,
    visit(node, ctx) {
        if (Node.isCallExpression(node)) {
...
function mountMethod(node, ctx) {
    ...
    if (!Node.isIdentifier(host) || !looksLikeRouter(host.getText(), ctx))
        return;
    ...
    const forwards = fn.getDescendantsOfKind(SyntaxKind.CallExpression).some((call) => {
        ...
        if (receiver.includes('.') || !looksLikeRouter(receiver, ctx))
            return false;
...
function paramPositions(fn) {
    ...
    params.forEach((param, index) => {
        ...
        if (param.isRestParameter())
            restName = nameNode.getText();
        else
            positions.set(nameNode.getText(), { from: 'start', index });
    });
    if (!restName)
        return positions;
        ...
        if (Node.isArrayBindingPattern(nameNode) && initializer.getText() === restName) {
            nameNode.getElements().forEach((element, index) => {
                ...
                if (Node.isIdentifier(local))
                    positions.set(local.getText(), { from: 'start', index });
            });
            continue;
        }
        if (!Node.isIdentifier(nameNode) || !Node.isElementAccessExpression(initializer))
            continue;
        if (initializer.getExpression().getText() !== restName)
            continue;
        const position = indexFromAccess(initializer.getArgumentExpression(), restName);
        if (position)
            positions.set(nameNode.getText(), position);
    }
    return positions;
}
/** `args[2]` and `args[args.length - 1]` — a position from either end of the list. */
function indexFromAccess(index, restName) {
    if (!index)
        return null;
    if (Node.isNumericLiteral(index))
        return { from: 'start', index: index.getLiteralValue() };
    if (!Node.isBinaryExpression(index))
        return null;
    if (index.getOperatorToken().getKind() !== SyntaxKind.MinusToken)
        return null;
    if (index.getLeft().getText() !== `${restName}.length`)
        return null;
 ...
 */
function routeHelper(fn, name, ctx) {
    const body = fn.getBody();
    if (!body)
        return;
    ...
    if (!router || !pathArg || !verb || templates.length === 0)
        return;
    ctx.emit({
        type: 'route-helper',
        name,
 ...
 */
function pathTemplate(arg, positions) {
    if (!arg)
        return null;
    ...
    if (spans[0].getLiteral().getLiteralText() !== '')
        return null;
    return { at, shape: `${arg.getHead().getLiteralText()}{}` };
}
/** The handler, through the one wrapper a helper usually puts round it. */
function handlerPosition(arg, positions) {
    if (!arg)
        return null;
    if (Node.isIdentifier(arg))
        return positions.get(arg.getText()) ?? null;
    ...
    if (Node.isCallExpression(arg)) {
        ...
        if (inner && Node.isIdentifier(inner))
            return positions.get(inner.getText()) ?? null;
    }
    return null;
}
/**
 ...
 */
function helperGuards(call, ctx) {
    const guards = [];
    const seen = new Set();
    const consider = (node) => {
        ...
        if (guard)
            guards.push({ ...guardAt(guard, call.getStartLineNumber()), how: 'middleware' });
    };
    const expand = (node, depth) => {
        ...
        if (Node.isSpreadElement(node)) {
            ...
            for (const declared of arrayBehind(inner))
                expand(declared, depth + 1);
            return;
        }
        consider(node);
    };
...
function helperRouteCall(call, ctx) {
    ...
    if (!literals.some((value) => value !== null && value.startsWith('/')))
        return;
    ctx.emit({
 ...
 */
function pathConstant(decl, ctx) {
...
function routerMount(call, ctx) {
    ...
    if (!looksLikeRouter(hostVar, ctx) && !handedOver(call.getExpression()))
        return;
    const args = call.getArguments();
 ...
 */
function routerHandoff(call, ctx) {
    const callee = call.getExpression();
    ...
    if (dotted?.includes('.')) {
        ...
        if (receiver && isRouter(receiver, ctx.locals))
            return;
    }
    ...
    for (const arg of call.getArguments()) {
        ...
        if (!isRouter(hostVar, ctx.locals))
            continue;
        ctx.emit({
            type: 'router-handoff',
            path: ctx.ref.relPath,
 ...
 */
function handoffTarget(callee, ctx) {
...
function sequenceOf(call, sf) {
    ...
    if (fn)
        return { from: fn.getStartLineNumber(), to: fn.getEndLineNumber() };
...
function handedOver(callee) {
    ...
    for (const declaration of receiver.getSymbol()?.getDeclarations() ?? []) {
        ...
        if (source.some((node) => Node.isParameterDeclaration(node)))
            return true;
    }
    return false;
}
 ...
 */
function mountedRouter(arg, ctx) {
    if (Node.isIdentifier(arg))
        return mountTarget(arg.getText(), ctx);
    if (!Node.isCallExpression(arg))
        return null;
    // `require('./users')` — the module handed over whole.
    const direct = requireSpecifier(arg);
    if (direct)
        return { module: importedModule(ctx.ref.relPath, direct), varName: null };
    ...
    if (Node.isIdentifier(callee))
        return mountTarget(callee.getText(), ctx);
    return null;
}
/**
...
function requireSpecifier(call) {
    const callee = call.getExpression();
...
function mountTarget(name, ctx) {
    ...
    if (local && isRouterCallee(local.callee))
        return { module: null, varName: name };
    return null;
}
/**
 ...
 */
function importedModule(fromRelPath, specifier) {
 ...
 */
function withoutExtension(specifier) {
    return specifier.replace(/\.[cm]?[jt]sx?$/, '');
}
/** `app.setGlobalPrefix('api')` — one line that renames every route NestJS serves. */
function globalPrefix(call, ctx) {
    if (!dottedName(call.getExpression())?.endsWith('.setGlobalPrefix'))
        return;
 ...
 */
function serverFrameworks(ctx) {
    return SERVER_PACKAGES.filter(({ pkg }) => ctx.signals.packages.has(pkg) || ctx.packages.has(pkg)).map(({ name }) => name);
}
/** `app.post('/users', requireAuth, createUser)` */
function routeCall(call, ctx) {
    const dotted = dottedName(call.getExpression());
...
function fastifyRouteObject(call, ctx) {
    const dotted = dottedName(call.getExpression());
    if (!dotted?.endsWith('.route'))
        return;
 ...
 */
function nestClassChain(cls, ctx) {
    const name = cls.getName();
    if (!name)
        return;
    const guards = decoratorGuards(cls.getDecorator('UseGuards'), ctx);
    const bases = [cls.getExtends()?.getExpression().getText()].filter((base) => Boolean(base));
    ...
    for (const guard of guards)
        ctx.emit({ type: 'auth-checker', name: guard.name, guard });
    ctx.emit({
        type: 'auth-alias',
        name,
...
function nestController(cls, ctx) {
    if (!ctx.signals.packages.has('@nestjs/common'))
        return;
 ...
 */
function strapiPolicies(config, ctx) {
 ...
 */
export const strapiRoutesDetector = {
    id: 'strapi-routes',
...
function strapiInPlay(ctx) {
    for (const name of ctx.signals.packages) {
        if (name === 'strapi' || name.startsWith('@strapi/'))
            return true;
    }
 ...
 * deployment contract.
 */
export const edgeFunctionDetector = {
    ...
    visit(node, ctx) {
        ...
        if (dotted !== 'Deno.serve' && dotted !== 'serve')
            return;
...
function edgeFunctionName(relPath) {
    const match = /(?:^|\/)supabase\/functions\/([^/]+)\//.exec(relPath);
    return match ? match[1] : null;
}
function decoratorGuards(decorator, ctx) {
    if (!decorator)
        return [];
    return decorator
        .getArguments()
...
function declaresPublic(decorator) {
...
function middlewareGuards(call, ctx) {
    const guards = [];
...
function looksLikeRouter(name, ctx) {
    return isRouter(name, ctx.locals);
}
function frameworkFor(name, ctx) {
    const local = ctx.locals.get(name);
    if (local?.module) {
    ...
    if (local && /Hono/.test(local.callee))
        return 'Hono';
    if (local && /express|Router/.test(local.callee))
        return 'Express';
    if (local && /[Ff]astify/.test(local.callee))
        return 'Fastify';
    return null;
}
/**
 ...
 */
function guessHandlerId(call, ctx) {
...
export const trpcDetector = {
    ...
    visit(node, ctx) {
        ...
        if (!/procedure/i.test(base))
            return;
        const name = node.getName().replace(/^['"]|['"]$/g, '');
        const guards = /protected|private|auth|admin|signed/i.test(base)
            ? [
                {
                    name: base,
                    how: 'procedure',
...
function providerForGuardName(name, ctx) {
 ...
 * clothes.
 */
export function exportedDeclarations(sf) {
    const out = new Map();
    for (const fn of sf.getFunctions()) {
        if (fn.isExported() || fn.isDefaultExport())
            out.set(fn.getName() ?? 'default', fn);
    }
    for (const decl of sf.getVariableDeclarations()) {
        if (decl.getVariableStatement()?.isExported())
            out.set(decl.getName(), decl);
    }
    return out;
}
/** The declaration behind `export default` — a page's component, usually. */
export function defaultExport(sf) {
    for (const fn of sf.getFunctions()) {
 ...
 */
function recordedRoutes(node, ctx) {
    ...
    for (const method of node.getMethods()) {
        ...
        for (const call of method.getDescendantsOfKind(SyntaxKind.CallExpression)) {
            ...
            if (!field || !replayed.has(field))
                continue;
            const recorded = call.getArguments()[0];
            if (!recorded || !Node.isObjectLiteralExpression(recorded))
                continue;
 ...
 */
function replayedFields(node) {
    const fields = new Set();
    const registers = (body, element) => body.getDescendantsOfKind(SyntaxKind.CallExpression).some((call) => {
        const callee = call.getExpression();
        ...
        const target = through ? callee.getExpression() : callee;
        if (!Node.isElementAccessExpression(target) && !Node.isPropertyAccessExpression(target))
            return false;
        if (Node.isPropertyAccessExpression(target) && !HTTP_METHODS.includes(target.getName().toUpperCase()))
            return false;
        const args = through ? call.getArguments().slice(1) : call.getArguments();
        return args.some((arg) => Node.isPropertyAccessExpression(arg) &&
            arg.getExpression().getText() === element &&
            arg.getName() === 'path');
    });
    for (const call of node.getDescendantsOfKind(SyntaxKind.CallExpression)) {
        const callee = call.getExpression();
        if (!Node.isPropertyAccessExpression(callee) || callee.getName() !== 'forEach')
            continue;
        const field = thisField(callee.getExpression());
        if (!field)
            continue;
        ...
        if (element && body && registers(body, element))
            fields.add(field);
    }
    for (const loop of node.getDescendantsOfKind(SyntaxKind.ForOfStatement)) {
        ...
        if (field && binding && registers(loop.getStatement(), binding))
            fields.add(field);
    }
    return fields;
}
...
export const recordedRouteDetector = {
    id: 'recorded-routes',
    enabled: () => true,
    visit(node, ctx) {
        if (Node.isClassDeclaration(node))
            recordedRoutes(node, ctx);
    },
};
