import { INBOUND_ID, OUTBOUND_ID, makeEdgeId, makeEndpointId, makeFileId, makeServiceId, makeStoreId, makeTypeId, } from '../../model/types.js';
...
import { guardThroughHops, reachableGuards, servicesThroughPyWrappers, servicesThroughUrlHelpers, servicesThroughWrappers, } from './reach.js';
 ...
 */
const PUBLIC_ENV_PREFIX = /^(NEXT_PUBLIC_|EXPO_PUBLIC_|VITE_|REACT_APP_|GATSBY_|NUXT_PUBLIC_|VUE_APP_|STORYBOOK_|PUBLIC_)/;
/** Whether a variable name should be treated as holding a credential. */
function isSecretName(name) {
    if (PUBLIC_ENV_PREFIX.test(name))
        return false;
    ...
    if (/^ConnectionStrings(:|$)/i.test(name))
        return true;
 ...
 */
function declaredInTest(finding) {
    if (classifyZone(finding.site.path) !== 'test')
        return false;
...
function pathOf(finding) {
    if ('site' in finding && finding.site)
        return finding.site.path;
    if ('path' in finding && typeof finding.path === 'string')
        return finding.path;
    return '';
}
export function buildBoundaryGraph(raw) {
    const nodes = [];
    const edges = [];
    ...
    const checkers = input.findings.filter((f) => f.type === 'auth-checker');
    const decorated = input.findings.filter((f) => f.type === 'handler-decorator');
    ...
    for (const endpoint of endpoints.values())
        for (const id of endpoint.handlerIds)
            routed.add(id);
    const guards = [
        ...input.findings.filter((f) => f.type === 'guard'),
        ...decoratorGuards.filter((guard) => guard.nodeId !== null && !routed.has(guard.nodeId) && !guard.nodeId.startsWith('type:')),
    ];
    applyWebhookPromotion(endpoints, input.findings);
    applyHandlerWrites(endpoints, input.findings);
    applyGuards(endpoints, guards, reachableGuards(guards, input.references ?? [], input.nodeNames ?? new Map()), input.findings.filter((f) => f.type === 'router-build'), input.findings.filter((f) => f.type === 'router-mount'), input.findings.filter((f) => f.type === 'router-handoff'));
    applyHandlerDecorators(endpoints, decoratorGuards);
    applyDependencyGuards(endpoints, checkers, input.findings.filter((f) => f.type === 'auth-alias'), input.findings.filter((f) => f.type === 'router-build'), input.findings.filter((f) => f.type === 'router-mount'), input.findings.filter((f) => f.type === 'router-guard'), input.findings.filter((f) => f.type === 'path-guard'));
    ...
    applyHandlerBlindness(endpoints, input.findings.filter((f) => f.type === 'handler-blind'));
    nameCommandLineDoors(endpoints, input);
    const envEndpoint = collectEnv(input);
    if (envEndpoint)
        endpoints.set(envEndpoint.id, envEndpoint);
    const services = collectServices(input);
    const stores = collectStores(input);
    if (endpoints.size > 0)
        nodes.push(container(INBOUND_ID, 'Ways in', 'in', endpoints.size, input.appId));
    ...
    const declaredTableIds = new Map();
    const prisma = input.signals.prisma;
    if (prisma)
        for (const model of prisma.models)
            declaredTableIds.set(model.toLowerCase(), makeTypeId(prisma.path, model));
    ...
    for (const store of stores.values()) {
        ...
        if (store.meta.storeKind !== 'sql' && store.meta.storeKind !== 'nosql')
            continue;
        for (const [table, sites] of store.tableSites) {
            ...
            addEdges(edges, flowEdges(tableId, sites, false, input.knownNodeIds).map((edge) => ({
                ...edge,
                kind: 'references',
            })));
        }
 ...
 * lines of it.
 */
function routerKey(path, varName) {
 ...
 */
function nameCommandLineDoors(endpoints, input) {
    const entries = input.signals.entryPoints ?? [];
    for (const endpoint of endpoints.values()) {
        if (endpoint.meta.endpointKind !== 'cli' || endpoint.meta.route)
            continue;
        ...
        if (runner)
            endpoint.meta.route = `${runner} ${path}`;
    }
}
function collectEndpoints(input) {
    ...
    const add = (finding) => {
        ...
        if (existing) {
            ...
            if (!declaredInTest(finding))
                delete existing.meta.declaredInTest;
            ...
            for (const guard of finding.guards)
                existing.meta.guards.push(guard);
            if (finding.handlerId)
                existing.handlerIds.add(finding.handlerId);
            if (finding.handlerId && finding.handlerIsScope)
                existing.scopeIds.add(finding.handlerId);
            ...
            if (finding.schedule && !existing.meta.schedule)
                existing.meta.schedule = finding.schedule;
            for (const name of finding.paramTypes ?? [])
                existing.paramTypes.add(name);
            if (finding.routerVar)
                existing.routers.add(routerKey(finding.site.path, finding.routerVar));
            if (finding.handlerOwner)
                existing.owners.add(finding.handlerOwner);
            return;
        }
        merged.set(id, {
            id,
    ...
    for (const cron of input.signals.crons) {
        ...
        const target = [...merged.values()].find((endpoint) => endpoint.kind === 'http-route' && endpoint.meta.route === cron.route);
        if (target) {
            target.kind = 'cron';
...
function argAtPosition(position, list) {
 ...
 */
function helperRoutes(findings) {
    const doors = [];
    const add = (finding) => void doors.push(finding);
    const helpers = new Map();
    const calls = [];
    /** Variables this repo builds a router in, so a fragment can be told from an address. */
    const built = new Set();
    for (const finding of findings) {
        if (finding.type === 'router-build')
            built.add(`${finding.path}\0${finding.varName}`);
    }
    for (const finding of findings) {
        ...
        if (finding.type === 'route-helper')
            helpers.set(finding.name, finding);
        else if (finding.type === 'helper-route-call')
            calls.push(finding);
    }
    if (helpers.size === 0)
        return doors;
    for (const call of calls) {
        ...
        if (!route || !route.startsWith('/'))
            continue;
        const verb = 'literal' in helper.verb ? helper.verb.literal : (argAtPosition(helper.verb.at, call.args) ?? null);
 ...
 */
function addPublishedPortDoors(input, add) {
...
function publishedPortDoorName(port) {
    ...
    let what;
    if (port.hostPort)
        what = `${port.hostPort}${udp} ${where}`;
    else if (port.hostPortVar)
        what = `the port ${port.hostPortVar} is set to, ${where}`;
    ...
    // number to print and printing the container's would be printing the wrong one.
        ...
        what = `port ${port.containerPort}${udp} inside the container, on a host port Docker picks${port.bindAddress ? `, ${where}` : ''}`;
    return `${port.configPath} publishes ${what} → ${port.target}`;
}
function bindPhrase(bind) {
    if (bind === null)
        return 'on every interface';
    if (bind.startsWith('$'))
        return `on the address ${bind.slice(1)} is set to`;
    return `on ${bind} only`;
}
 ...
 */
function addWorkerDoors(input, add) {
    for (const worker of input.signals.workers) {
        ...
        const main = worker.declaredEntry;
 ...
 */
function addPostgrestDoors(input, add) {
    ...
    for (const table of schema.tables) {
        ...
        const guards = table.rlsEnabled
            ? [
                {
                    name: table.policies.length > 0
 ...
 */
function applyHandlerWrites(endpoints, findings) {
    const writers = new Set();
    for (const finding of findings) {
        if (finding.type !== 'store' || finding.operation !== 'write')
            continue;
        if (finding.site.nodeId)
            writers.add(finding.site.nodeId);
    }
    if (writers.size === 0)
        return;
    for (const endpoint of endpoints.values()) {
        if (endpoint.meta.writes)
            continue;
        for (const handlerId of endpoint.handlerIds) {
            if (!writers.has(handlerId))
                continue;
 ...
 */
function applyWebhookPromotion(endpoints, findings) {
    const verifiedFiles = new Map();
    for (const finding of findings) {
        if (finding.type === 'webhook')
            verifiedFiles.set(finding.site.path, finding.provider);
    }
    for (const endpoint of endpoints.values()) {
        if (endpoint.kind !== 'http-route')
            continue;
 ...
 */
function applyGuards(endpoints, guards, reached, builds, mounts, handoffs) {
    ...
    for (const endpoint of endpoints.values()) {
        ...
        const covers = (guard) => onRouter(endpoint, guard) &&
            (addressable
                ? guard.matchers.some((matcher) => matcherMatches(matcher, route) || isCatchAllMatcher(matcher))
        ...
        for (const guard of matchers) {
            if (guard.coversFrom && covers(guard) && above(endpoint, guard))
                gated.add(head(guard.guard.name));
        }
        for (const site of endpoint.meta.sites) {
            for (const guard of byFile.get(site.path) ?? []) {
                if (gated.has(head(guard.guard.name)))
                    continue;
        ...
        for (const guard of matchers) {
            ...
            // lock (#237).
            if (guard.parsesOnly) {
                if (!endpoint.meta.declaredInTest)
                    endpoint.meta.identityOnly ??= guard.guard.name;
                continue;
            }
 ...
 */
function handlerDecoratorGuards(decorators, checkers) {
 ...
 */
function applyHandlerBlindness(endpoints, blind) {
    if (blind.length === 0)
        return;
    const byNode = new Map(blind.map((finding) => [finding.nodeId, finding]));
    for (const endpoint of endpoints.values()) {
        if (endpoint.meta.guards.length > 0)
            continue;
...
function applyHandlerDecorators(endpoints, guards) {
    if (guards.length === 0)
        return;
    ...
    for (const guard of guards) {
        ...
        if (list)
            list.push(guard.guard);
        else
            byNode.set(guard.nodeId, [guard.guard]);
    }
    for (const endpoint of endpoints.values()) {
        for (const handlerId of endpoint.handlerIds) {
            for (const guard of byNode.get(handlerId) ?? [])
                pushGuard(endpoint, guard);
        }
    }
}
/**
    ...
    return (endpoint, guard) => {
        ...
        const reaches = (key) => {
            ...
            for (const mount of mountedAt.get(key) ?? []) {
                if (reaches(moduleRouterKey(moduleOf(mount.path), mount.hostVar)))
                    return true;
            }
            return false;
        };
...
function registeredAboveTheGate(mountedAt, builds, handoffs) {
    ...
    return (endpoint, guard) => {
        ...
        for (const key of endpoint.routers) {
            ...
            if (builtAt.has(owned))
                continue;
            const where = owned.slice(0, owned.indexOf('\0'));
            const handed = handoffs.filter((handoff) => handoff.path === gate.path &&
            ...
            if (handed.some((handoff) => handoff.line < gate.line))
                return true;
        }
        return false;
    };
 ...
 */
function routersMountedHere(mountedAt) {
    const resolved = new Set();
    for (const list of mountedAt.values()) {
        for (const mount of list) {
            if (mount.childName)
                resolved.add(`${mount.path}\0${mount.line}\0${mount.childName}`);
        }
    }
    if (resolved.size === 0)
        return () => false;
 ...
 */
function applyDependencyGuards(endpoints, checkers, aliases, routers, mounts, attached, byPath) {
    ...
    for (const alias of aliases)
        declaredTimes.set(alias.name, (declaredTimes.get(alias.name) ?? 0) + 1);
    for (const alias of aliases) {
        if (byName.has(alias.name))
            continue;
        if ((declaredTimes.get(alias.name) ?? 0) > 1)
            continue;
        const target = alias.depends.find((name) => byName.has(name));
        if (!target)
            continue;
        const inherited = byName.get(target);
    ...
    for (const build of routers) {
        ...
        if (guard)
            byRouter.set(routerKey(build.path, build.varName), { ...guard, how: 'config' });
    }
    ...
    for (const endpoint of endpoints.values()) {
        ...
        for (const key of endpoint.routers) {
            ...
            if (behindTheMount)
                pushGuard(endpoint, behindTheMount);
        }
        const route = endpoint.meta.route;
        for (const entry of route ? wired : []) {
            ...
            if (entry.method && entry.method !== endpoint.meta.method)
                continue;
            if (!matcherMatches(entry.matcher, route))
                continue;
            const guard = byName.get(entry.name.split('.').pop() ?? entry.name);
            pushGuard(endpoint, { ...guard, how: 'middleware', confidence: 'likely' });
        }
        for (const owner of endpoint.owners) {
            const guard = inherited(owner, endpoint.meta.sites.map((site) => site.path));
            ...
            if (guard)
                pushGuard(endpoint, { ...guard, how: 'config', confidence: 'likely' });
        }
    }
}
/**
 ...
 */
function checkInherited(aliases, byName) {
    const declared = new Map();
    const atFile = new Map();
    for (const alias of aliases) {
    ...
    return (className, sitePaths) => {
        for (const sitePath of sitePaths) {
            const local = atFile.get(`${sitePath}#${className}`);
            if (local)
                return fromAlias(local);
        }
 ...
 * tool could say.
 */
function routersBehindACheck(builds, mounts, attached, byName, switched) {
    if (mounts.length === 0 && attached.length === 0)
        return new Map();
    ...
    const readable = (names) => switched.size === 0 ? names : names?.filter((name) => !switched.has(name.split('.').pop() ?? name));
    ...
    const claim = (path, varName, names, how, line) => {
        ...
        if (list)
            list.push({ line, guard: entry });
        else
            timeline.set(key, [{ line, guard: entry }]);
    };
    for (const build of builds)
        claim(build.path, build.varName, build.dependencies, 'config', build.line);
    for (const guard of attached)
        claim(guard.path, guard.varName, guard.names, guard.how, guard.line);
    const mountedAt = mountGraph(builds, mounts);
    const answers = new Map();
    /** What this router carried by then, or everything it carries when the question is not asked. */
    const carried = (key, asOf) => {
        if (asOf === null)
            return own.get(key) ?? null;
        let best = null;
        for (const entry of timeline.get(key) ?? []) {
            if (entry.line >= asOf)
                continue;
            if (!best || entry.line > best.line)
                best = entry;
        }
        return best?.guard ?? null;
    };
    const guardFor = (key, seen, asOf) => {
        ...
        if (!found) {
            ...
            if (parents.length > 0) {
                const guards = parents.map((mount) => {
                    ...
                    return guardFor(routerKey(moduleOf(mount.path), mount.hostVar), seen, mount.inheritsInOrder === true ? mount.line : asOf);
                });
 ...
 */
const CONFIDENCE_RANK = { possible: 0, likely: 1, certain: 2 };
function weaker(a, b) {
    return CONFIDENCE_RANK[a] <= CONFIDENCE_RANK[b] ? a : b;
}
function guardConfidence(endpoint, guard) {
    if (guard.nodeId && endpoint.handlerIds.has(guard.nodeId)) {
        ...
        if (!endpoint.scopeIds.has(guard.nodeId))
            return 'certain';
        return null;
    }
    // A module-scope check runs for everything the file declares.
    if (guard.nodeId?.startsWith('file:'))
        return 'likely';
...
function pushGuard(endpoint, guard) {
    ...
    const already = endpoint.meta.guards.find((g) => head(g.name) === head(guard.name) ||
        (g.path !== null && g.path === guard.path && g.line !== null && g.line === guard.line));
    if (already) {
        if (already.confidence === 'likely' && guard.confidence === 'certain')
            already.confidence = 'certain';
...
function collectEnv(input) {
    ...
    for (const finding of input.findings) {
        ...
        if (entry) {
            ...
            byName.set(finding.name, { sites: [finding.site], config: Boolean(finding.config) });
    }
    if (byName.size === 0)
        return null;
...
function collectServices(input) {
    ...
    for (const finding of input.findings) {
        ...
        if (existing) {
            ...
            if (finding.host && !existing.meta.hosts.includes(finding.host))
                existing.meta.hosts.push(finding.host);
 ...
 */
function addWorkerBindings(input, merged) {
    for (const worker of input.signals.workers) {
        if (!isWorker(worker))
            continue;
...
function collectStores(input) {
    ...
    for (const finding of input.findings) {
        if (finding.type === 'store' && finding.declares && finding.table)
            declaredTables.add(finding.table);
    }
    const resolveTable = (finding) => {
        if (finding.table || !finding.tableReceiver)
            return finding.requiresTable && !finding.table ? null : finding;
        for (const segment of finding.tableReceiver.split('.').reverse()) {
            if (declaredTables.has(segment.trim()))
                return { ...finding, table: segment.trim() };
        }
    ...
    const noteTable = (store, table, site) => {
        if (!table)
            return;
        ...
        if (!seen)
            store.meta.tables.push(table);
        const name = seen ?? table;
        const sites = store.tableSites.get(name);
        if (sites)
            sites.push(site);
        else
            store.tableSites.set(name, [site]);
    };
    ...
    if (prisma && input.signals.prisma) {
        for (const model of input.signals.prisma.models) {
            if (!prisma.meta.tables.includes(model))
                prisma.meta.tables.push(model);
        }
    }
 ...
 */
function nameTheClient(store, finding) {
    if (finding.generic)
        return;
    if (store.generic) {
        store.generic = false;
    ...
    if (named.includes(finding.client) || named.length >= 3)
        return;
 ...
 * "Database".
 */
function nameTheAnonymousDatabase(merged) {
    ...
    for (const store of anonymous) {
        ...
        for (const table of store.meta.tables) {
            if (!target.meta.tables.includes(table))
                target.meta.tables.push(table);
        }
        for (const table of store.meta.catalogTables) {
            if (!target.meta.catalogTables.includes(table))
                target.meta.catalogTables.push(table);
        }
        for (const [table, sites] of store.tableSites) {
            const existing = target.tableSites.get(table);
            if (existing)
                appendAll(existing, sites);
            else
                target.tableSites.set(table, sites);
        }
...
// Nodes
// ---------------------------------------------------------------------------
function container(id, name, direction, count, appId) {
    return {
        id,
        kind: 'zone',
        name,
 ...
 */
function observedTableNode(store, table, sites) {
    const first = sites[0];
    return {
...
function endpointEdges(endpoint, known) {
    const out = [];
    for (const handlerId of endpoint.handlerIds) {
        if (!known.has(handlerId))
            continue;
        out.push({
            kind: 'exposed-by',
            fromId: endpoint.id,
...
function flowEdges(targetId, sites, writes, known) {
    ...
    return [...counts.entries()].map(([fromId, weight]) => ({
        kind: writes ? 'writes-to' : 'reads-from',
        fromId,
        toId: targetId,
        weight,
        confidence: 'likely',
