...
import { refusalDetector, remixRoutesDetector, svelteHooksDetector, svelteRoutesDetector, } from './fileroutes.js';
import { edgeFunctionDetector, expoRoutesDetector, nextRoutesDetector, nodeRoutesDetector, recordedRouteDetector, routeHelperDetector, strapiRoutesDetector, trpcDetector, } from './http.js';
...
export function detectBoundaries(input) {
    ...
    for (const binding of imports.values()) {
        if (binding.external)
            packages.add(binding.module);
    }
    for (const specifier of unboundRequires(input.sf))
...
function buildImports(sf) {
    ...
    const add = (local, specifier, imported) => {
    ...
    for (const decl of sf.getImportDeclarations()) {
        ...
        if (def)
            add(def.getText(), specifier, 'default');
        const ns = decl.getNamespaceImport();
        if (ns)
            add(ns.getText(), specifier, '*');
    ...
    for (const decl of sf.getVariableDeclarations()) {
        ...
        if (!init || !Node.isCallExpression(init))
            continue;
        if (init.getExpression().getText() !== 'require')
            continue;
 ...
 */
function buildLocals(sf, imports) {
    ...
    for (const decl of sf.getDescendantsOfKind(SyntaxKind.VariableDeclaration)) {
        ...
        if (!init || !(Node.isCallExpression(init) || Node.isNewExpression(init)))
            continue;
 ...
 */
function unboundRequires(sf) {
    const found = [];
    for (const call of sf.getDescendantsOfKind(SyntaxKind.CallExpression)) {
        const callee = call.getExpression();
        const isRequire = Node.isIdentifier(callee) && callee.getText() === 'require';
        if (!isRequire && callee.getKind() !== SyntaxKind.ImportKeyword)
            continue;
        const specifier = call.getArguments()[0];
        if (!specifier || !Node.isStringLiteral(specifier))
            continue;
 ...
 */
function calleeThroughRequire(expression) {
    if (!Node.isPropertyAccessExpression(expression))
        return null;
    const base = expression.getExpression();
    if (!Node.isCallExpression(base))
        return null;
    if (base.getExpression().getText() !== 'require')
        return null;
