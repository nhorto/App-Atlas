 ...
 * one worth keeping.
 */
export function cleanSentence(raw, maxWords = 24) {
...
export function cleanLabel(raw, fallbackName) {
    if (typeof raw !== 'string')
        return null;
    const text = unwrap(raw).replace(/\s+/g, ' ').replace(/[.:;]+$/, '');
    if (!text || looksLikeFailure(text))
        return null;
    if (countWords(text) > 4 || text.length > 34)
        return null;
...
export function cleanParagraph(raw, maxSentences = 6) {
    if (typeof raw !== 'string')
        return null;
 ...
 */
export function methodsByRoute(routes) {
    const table = new Map();
    for (const { method, route } of routes) {
        ...
        if (verbs)
            ...
            table.set(key, new Set([(method ?? '').toUpperCase()]));
    }
    return table;
}
/**
 ...
 * table.
 */
export function dropWrongMethods(text, routes) {
    if (routes.size === 0)
        return { text, wrong: [] };
    const wrong = [];
...
function disprovenClaims(sentence, routes) {
    const disproved = [];
    for (const match of sentence.matchAll(PATH_MENTION)) {
        ...
        for (const claimed of claimedMethods(sentence, match.index, match[0].length)) {
            if (!verbs.has(claimed))
                disproved.push(`${claimed} ${mention}`);
        }
    }
    return disproved;
}
 ...
 */
function verbsFor(mention, routes) {
    ...
    for (const [known, methods] of routes) {
        ...
        if (knownParts.length !== wantedParts.length)
            continue;
        if (!knownParts.every((part, index) => segmentMatches(wantedParts[index], part)))
            continue;
        found = true;
 ...
 */
function segmentMatches(mentioned, known) {
    if (known === '{}')
        return true;
    if (mentioned === '{}')
        return false;
 ...
 */
function claimedMethods(sentence, index, length) {
    const claimed = [];
    for (const word of wordsIn(sentence.slice(0, index)).slice(-WORDS_BEFORE).reverse()) {
        const method = asMethod(word);
        if (method)
            claimed.push(method);
        else if (!FILLER.has(word.toLowerCase()) && !JOINERS.has(word.toLowerCase()))
            break;
    }
...
const EXTENSIONS = 'py|pyi|ipynb|ts|tsx|js|jsx|mjs|cjs|json|jsonc|md|mdx|toml|yaml|yml|sql|css|scss|html|sh|env|lock|txt|rs|go|rb|java|kt|swift|php|cs|prisma';
/**
 ...
 */
export function dropUngroundedFiles(text, allowed) {
    const known = new Set();
    for (const path of allowed) {
        known.add(path.toLowerCase());
        const base = path.split('/').pop();
        if (base)
            known.add(base.toLowerCase());
    }
    const wrong = [];
    const kept = text.split(SENTENCE_BREAK).filter((sentence) => {
 ...
 */
const FILE_MENTION = /\b[\w.@()[\]-]+(?:\/[\w.@()[\]-]+)*\.(?:tsx?|jsx?|mjs|cjs|py|go|rs|cs|java|kt|rb|php|swift|vue|svelte)\b/g;
