...
import { cleanLabel, cleanParagraph, cleanSentence, dropWrongMethods, methodsByRoute, parseJsonReply, } from './validate.js';
...
export async function enrichAtlas(options) {
    ...
    const routes = methodsByRoute(atlas.nodes
    ...
    for (const item of groupItems) {
        ...
        if (node)
            applyModule(node, hit.text, item.group.fileCount, routes, report);
        report.reusedFromCache++;
    }
    for (const item of folderItems) {
        ...
        if (node)
            applyModule(node, hit.text, null, routes, report);
        report.reusedFromCache++;
    }
    for (const item of fileItems) {
        ...
        const node = byId.get(item.item.key);
        if (node && applySummary(node, hit.text, routes, report))
            report.described++;
        report.reusedFromCache++;
    }
    const hashOf = new Map();
    for (const entry of groupItems)
        hashOf.set(entry.group.id, entry.hash);
    for (const entry of [...folderItems, ...fileItems])
        hashOf.set(entry.item.key, entry.hash);
 ...
 */
function companiesNotShown(paragraph, shown) {
    const onScreen = shown.join('\n');
    const missing = [];
    for (const name of knownServiceNames()) {
        if (!mentions(paragraph, name))
            continue;
        if (mentions(onScreen, name))
            continue;
        missing.push(name);
    }
    return missing;
...
function applyReply(job, text, byId, routes, report) {
    ...
    for (let index = 0; index < job.nodeIds.length; index++) {
        ...
        if (applySummary(node, sentence, routes, report))
            report.described++;
    }
}
/**
 ...
 */
function applyModule(node, stored, describes, routes, report) {
    let entry;
    try {
    ...
    if (describes !== null && (label || sentence))
        node.meta.describedFileCount = describes;
    if (label) {
        node.label = label;
        report.labelled++;
    }
    if (sentence && applySummary(node, sentence, routes, report))
        report.described++;
 ...
 */
function applySummary(node, text, routes, report) {
    if (node.summarySource === 'docs')
        return false;
    const grounded = dropWrongMethods(text, routes);
    for (const claim of grounded.wrong) {
        if (!report.misattributedRoutes.includes(claim))
            report.misattributedRoutes.push(claim);
    }
    if (!grounded.text)
        return false;
...
function remember(report, tier, nodeId, hash, text) {
...
function responsibilitiesByPath(atlas) {
    const map = new Map();
    const add = (path, what) => {
        if (!path)
            return;
        ...
        if (list) {
            if (list.length < 8 && !list.includes(what))
                list.push(what);
        }
        else {
            map.set(path, [what]);
    ...
    for (const node of atlas.nodes) {
        if (node.kind === 'endpoint') {
            ...
            const meta = node.meta;
            for (const site of meta.sites ?? [])
                add(site.path, `calls ${node.name}`);
        }
        else if (node.kind === 'store') {
            const meta = node.meta;
            for (const site of meta.sites ?? [])
                add(site.path, `uses ${node.name}`);
        }
    }
    return map;
}
 ...
 */
export function collectAppFacts(atlas, groups) {
    const waysIn = [];
    const services = [];
    const stores = [];
    for (const node of atlas.nodes) {
        if (node.kind === 'endpoint') {
            ...
            if (meta.endpointKind === 'env')
                continue;
            if (waysIn.length < 14)
                waysIn.push(meta.route ? `${meta.method ?? meta.endpointKind} ${meta.route}` : node.name);
        }
    ...
    const outline = live.slice(0, MAX_GROUPS_IN_OVERVIEW).map((group) => ({
        path: group.path,
 ...
 */
function collectGroupItems(groups, byId) {
    return groups
 ...
 */
function collectFolderItems(atlas, byId, responsibilities, isGroup) {
 ...
 */
function collectFileItems(atlas, responsibilities, maxFiles) {
    ...
    const exportsOf = (node) => node.meta.exportedNames ?? [];
    ...
    const items = chosen.map((node) => {
        const item = {
            ...
            contains: [
                ...
                ...(node.meta.externalImports ?? []).slice(0, 6).map((p) => `imports ${p}`),
            ],
 ...
 */
function factSignature(facts) {
    return JSON.stringify(facts);
}
// ---------------------------------------------------------------------------
// Plumbing
// ---------------------------------------------------------------------------
function estimateFor(backend, jobs, overview, items) {
    let inputTokens = 0;
    let outputTokens = 0;
