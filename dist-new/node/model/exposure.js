 ...
 */
export function classifyOpenDoors(nodes, edges) {
    ...
    for (const node of nodes) {
        switch (node.kind) {
            case 'endpoint': {
                ...
                if (isAuthRelevant(meta) && meta.guards.length === 0)
                    open.push(node);
                if (meta.signInCall)
                    signInDoors++;
                break;
            ...
            case 'store':
                ...
                break;
            case 'service':
                if (node.meta.category === 'auth')
                    authServices.set(node.id, node.name);
                break;
            case 'file': {
                if (node.meta.unread)
                    unreadable.set(node.id, node.path ?? node.name);
                ...
                if (typeof provider === 'string' && provider)
                    authFiles.set(node.id, provider);
                break;
            }
            default:
                break;
    ...
    const authMounts = authServices.size > 0 || authFiles.size > 0 ? authMountFiles(edges, authServices, authFiles) : null;
    ...
    for (const node of open) {
        ...
        if (meta.handlerUnlinked) {
            verdicts.set(node.id, {
                kind: 'unlinked',
                because: meta.handlerUnlinkedWhy ??
 ...
 */
function nothingJudged(routes, unlinked, inTest) {
    const subject = routes === 1 ? 'the one route is' : `all ${routes} routes are`;
    const unfollowed = 'declared in a routing table App Atlas has not followed to its handler — ' +
        'no auth verdict was reached for any of them';
    if (inTest === 0)
        return `${subject} ${unfollowed}`;
    if (unlinked === 0) {
        return `${subject} declared by the test suite — nothing here answers a URL in a deployed app`;
    }
    return (`no route was judged: ${inTest} ${inTest === 1 ? 'is' : 'are'} declared by the test suite, and ` +
        `${unlinked} ${unlinked === 1 ? 'is' : 'are'} in a routing table App Atlas has not followed to a handler`);
}
export function tallyOpenDoors(verdicts) {
    const tally = {
        worthALook: 0,
        page: 0,
        authMount: 0,
    ...
    for (const verdict of verdicts) {
        if (verdict.kind === 'page')
            tally.page++;
        else if (verdict.kind === 'auth-mount')
            tally.authMount++;
        else if (verdict.kind === 'unreadable')
            tally.unreadable++;
        else if (verdict.kind === 'generated')
            tally.generated++;
        else if (verdict.kind === 'unlinked')
            tally.unlinked++;
        else if (verdict.kind === 'declared-public')
            tally.declaredPublic++;
        else if (verdict.kind === 'in-test')
            tally.inTest++;
        else if (verdict.kind === 'identity-only')
            tally.identityOnly++;
        else
            tally.worthALook++;
    }
 ...
 */
export function authHeadline(stats) {
    const { routes } = stats;
    ...
    if (routes === 0) {
        if (backbone) {
            return {
                tone: 'warn',
                headline: `most of this repository is ${backbonePhrase(backbone)}, which App Atlas cannot read — ` +
        ...
        if (unreadFrameworks.length > 0) {
            return {
                tone: 'warn',
                headline: `this project declares ${unreadFrameworks.join(', ')}, whose routes App Atlas does not read — ` +
                    'whether anything answers a URL was never in view',
                caveats: [],
                hedged: false,
            };
        ...
        return {
            tone: 'warn',
            headline: `no routes were found — but App Atlas could not read ${unread} ${unread === 1 ? 'file' : 'files'}, ` +
    ...
    const caveats = [];
    if (open > 0 && unknown > 0) {
        caveats.push(`${unknown} more lean on a file App Atlas could not read — the check may well be in there`);
    }
    if (public_ > 0 && !mentionedPublic) {
        caveats.push(`${public_} more are pages or the door people sign in through, open on purpose`);
    }
    if (unlinked > 0) {
        caveats.push(`${unlinked} more ${unlinked === 1 ? 'is' : 'are'} declared in a routing table App Atlas has not followed to ` +
    ...
    if (inTest > 0) {
        caveats.push(`${inTest} more ${inTest === 1 ? 'is' : 'are'} declared by the test suite rather than by the app; ` +
            `${inTest === 1 ? 'it is' : 'they are'} in no number above`);
    }
    if (unread > 0) {
        caveats.push(`App Atlas could not read ${unread} ${unread === 1 ? 'file' : 'files'}; whatever they declare is missing from every number here`);
    }
    ...
    if (backbone) {
        caveats.push(`most of this repository is ${backbonePhrase(backbone)}, which App Atlas cannot read — ` +
            'the routes above are only the ones written in languages it can');
    }
    if (hedged) {
        caveats.push(likelyOnly === assessed - public_
...
export function unreadableFiles(nodes) {
    const list = [];
    for (const node of nodes) {
        if (node.kind !== 'file' || !node.meta.unread)
            continue;
 ...
 */
function filesBehind(node, meta) {
    const paths = new Set();
    if (node.path)
        paths.add(node.path);
...
function edgesByKind(edges, kind) {
    const map = new Map();
 ...
 */
function firstUnreadImport(files, importsOf, unreadable) {
    for (const fileId of files) {
        if (unreadable.has(fileId))
            return unreadable.get(fileId);
...
function authMountFiles(edges, authServices, authFiles) {
    const mounts = new Map(authFiles);
    for (const edge of edges) {
        const provider = authServices.get(edge.toId);
        if (!provider || !edge.fromId.startsWith('file:'))
            continue;
        mounts.set(edge.fromId, provider);
    }
    return mounts;
}
function providerMountedIn(files, mounts) {
    for (const fileId of files) {
 ...
 */
function doorsWritingData(edges, stores) {
    const doors = new Set();
    if (stores.size === 0)
        return doors;
    const writers = new Set();
    for (const edge of edges) {
        if (edge.kind === 'writes-to' && stores.has(edge.toId))
            writers.add(edge.fromId);
    }
    if (writers.size === 0)
        return doors;
    for (const edge of edges) {
        if (edge.kind === 'exposed-by' && writers.has(edge.toId))
            doors.add(edge.fromId);
    }
    return doors;
}
/**
 ...
 */
function becauseSignIn(call) {
    const why = call.what === 'sign-out'
