 ...
 */
export function exampleTrace(graph) {
    const chosen = pickFrames(graph);
    if (!chosen)
        return null;
    const { language, nodes } = chosen;
    const dialect = DIALECTS[language];
    const lines = nodes.map((node) => dialect.frame(node.name, node.path, node.startLine));
    return {
...
function pickFrames(graph) {
    ...
    const score = (node) => graph.edgesTo(node.id).filter((edge) => edge.kind === 'references').length;
...
function usable(node) {
    if (node.kind !== 'function')
        return null;
    if (node.zone === 'test' || !node.path || node.startLine === null)
        return null;
    if (isParked(node.path))
        return null;
...
function dialectOf(path) {
    ...
    for (const [language, dialect] of Object.entries(DIALECTS)) {
        if (dialect.extensions.includes(extension))
            return language;
    }
    return null;
}
...
//# sourceMappingURL=exampletrace.js.map