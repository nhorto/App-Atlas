 ...
 */
export function bundleMaps(root) {
...
function load(absolute, relative) {
    let key;
    try {
 ...
 */
export function parseSourceMap(text, mapPath = '') {
    let raw;
    try {
        raw = JSON.parse(text);
    }
...
class SectionedMap {
    ...
    originalFor(line, column) {
        for (let index = this.sections.length - 1; index >= 0; index--) {
            const section = this.sections[index];
            if (line < section.line || (line === section.line && column < section.column))
                continue;
            return section.map.originalFor(line - section.line, line === section.line ? column - section.column : column);
        }
        return null;
    }
}
...
class PlainMap {
    ...
    brokenFrom = null;
    constructor(mappings, sources, names) {
    ...
    rowAt(line) {
        if (line < 0 || line >= this.rows.length)
            return null;
        if (this.brokenFrom !== null && line >= this.brokenFrom)
            return null;
...
const DIGITS = new Map([...BASE64].map((char, value) => [char, value]));
/**
 ...
 */
export function looksBuilt(cleanedPath, line, column) {
    void line;
    const lower = cleanedPath.toLowerCase();
    if (BUILD_DIRS.some((dir) => lower.includes(`/${dir}`) || lower.startsWith(dir)))
        return true;
    if (BUILT_NAMES.some((pattern) => pattern.test(basename(lower))))
        return true;
    return (column ?? 0) >= 200;
}
