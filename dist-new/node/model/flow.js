 ...
 */
export function listDoors(graph) {
    ...
    for (const node of graph.nodesOfKind('endpoint')) {
        ...
        if (bucket)
            ...
            groups.set(door.endpointKind, [door]);
    }
 ...
 */
export function buildFlow(graph, doorId) {
    ...
    for (let hop = 2; hop <= MAX_HOPS; hop++) {
        ...
        if (next.length === 0)
            break;
        if (hop === MAX_HOPS && reachesFurther(graph, next, seen))
            limits.hitDepth = true;
        frontier = next;
    }
...
function asStop(node, hop, confidence) {
    return {
        id: node.id,
        name: node.name,
 ...
 */
function reachesFurther(graph, frontier, seen) {
    for (const node of frontier) {
        for (const edge of graph.edgesFrom(node.id)) {
            if (edge.kind !== 'references' || seen.has(edge.toId))
                continue;
            const target = graph.getNodeById(edge.toId);
            if (target && isCode(target))
                return true;
 ...
 */
function exitsFrom(graph, stops, limits) {
    const found = new Map();
    for (const stop of stops) {
        for (const edge of graph.edgesFrom(stop.id)) {
            ...
            if (already) {
                if (!already.reachedBy.includes(stop.id))
                    already.reachedBy.push(stop.id);
            ...
            found.set(target.id, {
                ...
                external: target.kind === 'service' ? (target.meta.external ?? true) : true,
                reachedBy: [stop.id],
            });
        }
    }
    // Somewhere data is written outranks somewhere it is only read.
    return [...found.values()].sort((a, b) => Number(b.writes) - Number(a.writes) || a.name.localeCompare(b.name));
}
function detailOf(node) {
    if (node.kind === 'store') {
        ...
        if (tables > 0)
            bits.push(`${tables} ${tables === 1 ? 'table' : 'tables'}`);
        return bits.filter(Boolean).join(' · ');
    }
    const meta = node.meta;
    const bits = [];
    if (meta.category && meta.category !== 'other')
        bits.push(meta.category);
    if (meta.hosts.length > 0)
        bits.push(meta.hosts[0]);
    return bits.join(' · ');
}
