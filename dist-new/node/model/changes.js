 ...
 */
export function diffAtlas(previous, next) {
    if (!previous) {
        return {
            baseline: 'none',
            because: null,
    ...
    for (const node of next.nodes) {
        ...
        if (!old)
            count(node.kind, 'added');
        else if (old.hash !== node.hash)
            count(node.kind, 'changed');
    }
    for (const node of previous.nodes) {
        if (!after.has(node.id))
            count(node.kind, 'removed');
    }
    return {
        baseline: 'compared',
        because: null,
 ...
 */
function whyNotComparable(before, after) {
 ...
 */
function diffDoors(previousNodes, nextNodes, before, after) {
    ...
    for (const node of nextNodes) {
        ...
        if (!old) {
            doors.newTotal++;
            if (meta.open?.kind === 'worth-a-look')
                doors.newOpen.push(toDoorChange(node, meta));
            continue;
        }
        const oldMeta = old.meta;
        const wasChecked = (oldMeta.guards ?? []).length > 0;
        if (wasChecked && (meta.guards ?? []).length === 0)
            doors.lostCheck.push(toDoorChange(node, meta));
    }
    for (const node of previousNodes) {
        if (node.kind !== 'endpoint' || after.has(node.id))
            continue;
        const meta = node.meta;
        if (!isAuthRelevant(meta))
            continue;
...
function toDoorChange(node, meta) {
...
export function describeChanges(changes) {
    ...
    if (changes.baseline === 'incomparable') {
        return say('muted', `nothing to compare against — ${changes.because ?? 'the atlas on disk was written under different conditions'}`, ['a diff across that gap would be noise rather than news, so the next run is the one that will say what changed']);
    }
    ...
    if (doors.lostCheck.length > 0) {
        notes.push({
            text: `${plural(doors.lostCheck.length, 'door', 'doors')} that had an auth check last run ${doors.lostCheck.length === 1 ? 'has' : 'have'} none now`,
            doors: doors.lostCheck,
        });
    }
    if (doors.removed.length > 0) {
        notes.push({
            text: `${plural(doors.removed.length, 'door', 'doors')} that ${doors.removed.length === 1 ? 'was' : 'were'} here last run ${doors.removed.length === 1 ? 'is' : 'are'} gone`,
            doors: doors.removed,
        });
    }
    ...
    if (notes.length === 0) {
        notes.push({
            text: total.added === 0 && total.removed === 0 && total.changed === 0
                ...
                : doors.newTotal > 0
                    ? // Deliberately not "and something checks them". A new door can also be an
                        ...
                        `${plural(doors.newTotal, 'new door', 'new doors')} since the last run, and nothing about ${doors.newTotal === 1 ? 'it' : 'them'} is left unexplained`
                    : `${shape || 'a few things moved'} since the last run`,
            doors: [],
        });
    }
    if (shape && !notes[0].text.startsWith(shape))
        notes.push({ text: shape, doors: [] });
    return {
...
function say(tone, headline, lines) {
    return {
        tone,
...
function movement(changes) {
    ...
    for (const [kind, one, many] of KIND_NOUNS) {
        ...
        for (const phrase of [
            ...
            if (!phrase)
                continue;
            if (phrases.length < MAX_MOVEMENT_PHRASES)
                phrases.push(phrase);
            else
                dropped++;
        }
