...
function withAiOptions(command) {
    return command
...
program
    ...
    for (const result of results) {
        ...
        const verb = result.action === 'created' ? pc.green('created') : result.action === 'updated' ? pc.cyan('updated') : pc.dim('already up to date');
 ...
 */
async function produceAtlas(root, options, run) {
...
async function runAnalysis(dir, options) {
    const root = path.resolve(dir);
    ...
    const byName = (s) => s.id === options.scope || s.name === options.scope;
    ...
    if (scopes.length > 1)
        return runWorkspaceAnalysis(root, scopes, options, hidden.length);
...
async function runSingleAnalysis(root, options, repoRoot = root) {
    const quiet = Boolean(options.quiet);
    ...
    const { atlas, words, dbPath, jsonPath, ignoredNow } = await produceAtlas(root, options, {
        ...
        onProgress: (stage, done, total) => {
            ...
            if (!hintShown && stage === 'Reading files' && total > 800 && options.refs !== false) {
                hintShown = true;
                console.log(pc.dim(`  ${total} files — the first pass takes a minute. Use --no-refs for a quicker structural map.`));
            }
            if (!interactive) {
    ...
    if (!quiet) {
        ...
        if (tier)
            console.log(pc.dim(`  ${tier.display} read by grammar, not by a compiler — links between files are likely, not certain.`));
        ...
        if (backbone) {
            console.log(pc.yellow(`  most of this repository is ${backbonePhrase(backbone)}, which App Atlas cannot read — ` +
        ...
        if (auth) {
            ...
            for (const caveat of auth.caveats)
                console.log(pc.dim(`  ${caveat}`));
        }
        console.log('');
        ...
        const nudge = s.documentedFiles < ownFiles ? ` Run ${pc.cyan('app-atlas init')} to teach your agent to write them.` : '';
        const aside = generated > 0 ? `, ${generated} generated ${plural(generated, 'file', 'files')} aside` : '';
        if (ownFiles > 0) {
            console.log(pc.dim(`  ${documented}% of files have a docstring App Atlas can read (${s.documentedFiles}/${ownFiles}${aside}).${nudge}`));
        }
        if (s.staleDocs > 0) {
            console.log(pc.yellow(`  ${s.staleDocs} ${plural(s.staleDocs, 'docstring describes', 'docstrings describe')} code that has changed since it was written.`));
        }
        for (const line of words ? describeRun(words) : [])
        ...
        if (ignoredNow)
            console.log(pc.dim(`  added .app-atlas/ to .gitignore — the map is local to this machine`));
...
function printChanges(changes) {
    ...
    for (const line of report.lines)
        printNote(line, pc.dim);
}
function printNote(note, paint) {
    ...
    if (hidden > 0)
        console.log(paint(`    ...and ${hidden} more`));
}
 ...
 */
async function runWorkspaceAnalysis(root, scopes, options, 
...
hiddenTests = 0) {
    ...
    if (!quiet) {
        console.log(`  ${pc.bold(String(scopes.length))} ${plural(scopes.length, 'package', 'packages')} in this workspace` +
        ...
        if (hiddenTests > 0) {
            console.log(pc.dim(`  ${hiddenTests} more ${plural(hiddenTests, 'is a test fixture', 'are test fixtures')} the workspace ` +
    ...
    for (const scope of scopes) {
        try {
            ...
            if (!quiet)
                console.log(`  ${pc.bold(scope.name.padEnd(width))}  ${scopeLine(atlas)}`);
        }
        catch (err) {
            // One package that will not parse must not cost someone the other five.
            failures.push(`${scope.name}: ${err.message}`);
            if (!quiet)
                console.log(`  ${pc.bold(scope.name.padEnd(width))}  ${pc.yellow('could not be read')}`);
        }
    }
    if (results.length === 0)
        throw new Error(`None of the packages could be analyzed. ${failures[0] ?? ''}`);
    writeScopes(root, results.map(({ scope }) => scope));
    if (!quiet) {
        ...
        if (stack.length > 0) {
            ...
            console.log(`  ${pc.dim('the repo publishes')} ${pc.bold(String(stack.length))} ` +
            ...
            if (stack.length > 6)
                console.log(pc.dim(`    …and ${stack.length - 6} more`));
        }
        console.log('');
 ...
 */
function repoPublishedPorts(root, ignore) {
    const scopeDirs = readScopes(root)
        .map((scope) => scope.dir)
        .filter((dir) => dir && dir !== '.');
    return readComposePorts(root, root, buildIgnoreMatcher(root, ignore ?? [])).filter((port) => !scopeDirs.some((dir) => port.configPath === dir || port.configPath.startsWith(`${dir}/`)));
}
/** One line per app: size, doors, and the number worth interrupting for. */
function scopeLine(atlas) {
    const s = atlas.meta.stats;
    const parts = [`${s.files} ${plural(s.files, 'file', 'files')}`];
    if (s.endpoints > 0)
        parts.push(`${s.endpoints} ${plural(s.endpoints, 'way in', 'ways in')}`);
    if (s.services > 0)
        parts.push(`${s.services} ${plural(s.services, 'service', 'services')}`);
    ...
    if (assessed === 0) {
        const reason = inTest > unlinked ? 'declared by the test suite' : `not followed to a handler`;
...
async function runServer(dir, atlas, scopes, options) {
 ...
 */
function startWatching(dir, handle, options) {
    const root = path.resolve(dir);
...
program.parseAsync(process.argv).catch((err) => {
    console.error('');
    console.error(pc.red(`App Atlas failed: ${err.message}`));
    if (process.env.APP_ATLAS_DEBUG)
        console.error(err);
    process.exitCode = 1;
});
