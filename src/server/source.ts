...
export interface SourceSlice {
  ...
  endLine: number;
  code: string;
  /** True when the slice stops short of the declaration's real end. */
  truncated: boolean;
}

export function readSource(root: string, node: AtlasNode, maxLines = MAX_SOURCE_LINES): SourceSlice | null {
  if (!node.path || !node.startLine) return null;
