...
export const MINIMUM_NODE = { major: 22, minor: 5 } as const;
 ...
 * for everybody.
 */
export function nodeIsTooOld(version: string): boolean {
 ...
 */
export function isSqliteExperimentalWarning(warning: string | Error, type: unknown): boolean {
  const text = typeof warning === 'string' ? warning : (warning?.message ?? '');
  const name =
    typeof type === 'string' ? type : ((type as { type?: string } | undefined)?.type ?? '');
