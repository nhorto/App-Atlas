 ...
 */
export function initConventions(root: string, only?: string): InitResult[] {
...
function write(filePath: string): InitResult {
  ...
  if (start !== -1 && end > start) {
    ...
    if (current === BLOCK) return { path: filePath, action: 'unchanged' };
