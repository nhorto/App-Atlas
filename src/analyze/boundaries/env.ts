...
export const envDetector: BoundaryDetector = {
  id: 'env',
  enabled: () => true,
  visit(node, ctx) {
    if (Node.isPropertyAccessExpression(node)) {
      if (ENV_OBJECTS.has(node.getExpression().getText())) emit(node.getName(), node, ctx);
      return;
    }

    if (Node.isElementAccessExpression(node)) {
      if (!ENV_OBJECTS.has(node.getExpression().getText())) return;
      const name = literalString(node.getArgumentExpression());
      if (name) emit(name, node, ctx);
      return;
    }
    ...
    if (Node.isVariableDeclaration(node)) {
      ...
      if (!init || !Node.isObjectBindingPattern(name)) return;
      if (!ENV_OBJECTS.has(init.getText())) return;
    ...
    if (Node.isCallExpression(node)) {
      ...
      if (dotted === 'createEnv' || dotted === 'cleanEnv') {
        ...
        if (dotted === 'cleanEnv') collectKeys(config, node, ctx);
      }
    }
  },
};

function collectKeys(obj: Node | undefined, at: Node, ctx: DetectorContext): void {
...
function emit(name: string, at: Node, ctx: DetectorContext): void {
