...
function routeCall(call: CallExpression, ctx: DetectorContext): void {
  ...
  const bound = frameworkFor(holder, ctx);
  const framework = bound ?? frameworks[0] ?? 'HTTP';

  // Both rules below relax something, so both ask `bound` rather than `framework`: the
  // holder has to have been *resolved* to a Koa router, not merely live in a repository
  // that has Koa somewhere in its manifest. `looksLikeRouter` accepts a bare `app`,
  // `api`, `server` or `r` on the name alone, and `frameworks[0]` in a Koa-only repo
  // would hand every one of them the relaxation — turning `api.get('users')` on an HTTP
  // client into a way in. Requiring the binding costs nothing real: outline's routers are
  // `const router = new Router()` against an imported `koa-router`, which resolves.
  const koa = bound === 'Koa';
  ...
  const named = koa && literalString(argAt(call, 1)) !== null;
  ...
  // sails has exactly that line. Counted before relaxing it, on the rule this backlog
  // keeps re-learning — Ghost alone has 5,786 slashless calls on a router-shaped method,
  // nearly all of them `config.get('url')` and `Map.get`, so the check is load-bearing
  // and stays everywhere it is not disproved.
  //
  // Koa is where it is disproved: its router accepts a relative path, and its users write
  // them. outline declares 192 of its 226 routes as `router.post('documents.list', …)`
  // and reported 29 ways in out of 226 (#269).
