 ...
 */
export const GO_FRAMEWORKS: Record<string, string> = {
...
export function goFrameworkFor(module: string): string | null {
  for (const [prefix, label] of Object.entries(GO_FRAMEWORKS)) {
    if (module === prefix || module.startsWith(`${prefix}/`)) return label;
  }
  return null;
}
