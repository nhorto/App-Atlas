...
import type { EntryPoint, ProjectSignals } from './signals.js';

/** One directory a framework owns, and the name a reader knows that framework by. */
 ...
 */
function routeDirectories(signals: ProjectSignals): RouteDirectory[] {
  const out: RouteDirectory[] = [];
  const add = (dir: string | null, framework: string) => {
    if (dir && !out.some((entry) => entry.dir === dir)) out.push({ dir, framework });
  };
 ...
 */
const NAMED_FILES: { package: string; framework: string; paths: string[] }[] = [
  {
    package: 'next',
    framework: 'Next.js',
    paths: [
 ...
 */
function remixEntries(signals: ProjectSignals): string[] {
  const dir = signals.remixRoutesDir;
  if (!dir) return [];
  const parent = dir.includes('/') ? dir.slice(0, dir.lastIndexOf('/')) : '';
  const prefix = parent ? `${parent}/` : '';
  return ['root', 'entry.client', 'entry.server', 'routes'].flatMap((name) =>
    ['ts', 'tsx', 'js', 'jsx'].map((ext) => `${prefix}${name}.${ext}`),
  );
}
 ...
 * is none.
 */
export function frameworkOwnerOf(relPath: string, signals: ProjectSignals): string | null {
  for (const { dir, framework } of routeDirectories(signals)) {
    if (relPath === dir || relPath.startsWith(`${dir}/`)) return framework;
  }
  for (const entry of NAMED_FILES) {
    if (!signals.packages.has(entry.package)) continue;
    if (entry.paths.includes(relPath)) return entry.framework;
  }
  const remix = remixEntries(signals);
  if (remix.includes(relPath)) return 'Remix';
  return null;
}
 ...
 */
export function declaredEntryFor(relPath: string, entryPoints: EntryPoint[]): string | null {
  const stem = stemOf(relPath);
  for (const entry of entryPoints) {
    if (entry.path === relPath) return entry.field;
    if (stem && stemOf(entry.path) === stem) return entry.field;
  }
  return null;
}
