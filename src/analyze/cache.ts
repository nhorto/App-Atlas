...
import type { SliceRow } from '../model/store.js';
import { hashParts, hashText } from '../util/hash.js';
import type { FileSlice, PluginOptions } from './plugin.js';
import type { ProjectInfo, SourceFileRef } from './project.js';

export interface CachePlan {
  /** Files whose last result can be restored as-is. */
  reusable: Map<string, FileSlice>;
  /** Files that have to be read again. */
  stale: Set<string>;
  /** Text hash of every file in the project, computed on the way through. */
 ...
 */
export function fingerprintProject(project: ProjectInfo, toolVersion: string, options: PluginOptions): string {
...
export class AnalysisCache {
  ...
  static open(root: string, fingerprint: string): AnalysisCache | null {
    try {
   ...
   */
  plan(files: SourceFileRef[]): CachePlan {
    ...
    for (const file of files) {
      ...
      if (!cached || cached.hash !== hashes.get(file.relPath)) stale.add(file.relPath);
    }
    ...
    for (const [relPath, entry] of index) {
      if (stale.has(relPath) || !present.has(relPath)) continue;
      if (entry.imports.some((target) => changed.has(target))) stale.add(relPath);
    }
    ...
    const reusable = new Map<string, FileSlice>();
    for (const relPath of reusablePaths) {
      ...
      try {
        reusable.set(relPath, JSON.parse(json) as FileSlice);
      } catch {
        stale.add(relPath);
      }
    }
  ...
  save(slices: FileSlice[], keep: Set<string>): void {
