...
export interface MarkupFile {
  ...
  path: string;
  /** The fully-qualified class this markup completes, when it names one. */
  className: string | null;
  /** The root element's tag, without its namespace prefix. */
  root: string;
  /** True when the root is something a person looks at. */
  isScreen: boolean;
  /** Method names named by event attributes. */
  handlers: string[];
  /** Property names named by `{x:Bind …}` and `{Binding …}`. */
  bindings: string[];
  /** The first XML comment, when it sits above the content rather than inside it. */
  doc: string | null;
  loc: number;
}
...
export function readMarkupFiles(root: string, relPaths: string[]): MarkupFile[] {
  const out: MarkupFile[] = [];
...
export function readMarkup(relPath: string, text: string): MarkupFile | null {
  ...
  for (const match of text.matchAll(/\b([A-Za-z_][\w:.]*)\s*=\s*"([^"{}]*)"/g)) {
    ...
    if (NOT_A_HANDLER.has(attribute) || attribute.startsWith('xmlns')) continue;
    if (!HANDLER_VALUE.test(value)) continue;
    ...
    if (!handlers.includes(value)) handlers.push(value);
  }

  const bindings: string[] = [];
  for (const match of text.matchAll(/\{(?:x:Bind|Binding)\s+([A-Za-z_][\w.]*)/g)) {
    const name = match[1].split('.')[0];
    if (name && !bindings.includes(name)) bindings.push(name);
  }
 ...
 */
export function screenName(file: MarkupFile): string {
  if (file.className) return file.className.slice(file.className.lastIndexOf('.') + 1);
...
export function classNamespace(className: string): string {
 ...
 */
export function buildMarkupNodes(files: MarkupFile[], code: readonly AtlasNode[]): MarkupGraph {
  if (files.length === 0) return { nodes: [], edges: [], findings: [], filePaths: [] };
  ...
  const namespaceOfFile = new Map<string, string>();
  for (const node of code) {
    if (node.kind === 'file' && node.path && typeof node.meta.namespace === 'string') {
      namespaceOfFile.set(node.path, node.meta.namespace);
    }
  }
  const typeAt = new Map<string, AtlasNode>();
  for (const node of code) {
    if (node.kind !== 'type' || !node.path) continue;
    const key = `${namespaceOfFile.get(node.path) ?? ''}.${node.name}`;
    if (!typeAt.has(key)) typeAt.set(key, node);
  }
  const methodsOfOwner = new Map<string, AtlasNode[]>();
  for (const node of code) {
    if (node.kind !== 'function') continue;
    const owner = String((node.meta as { ownerName?: string }).ownerName ?? '');
    if (!owner) continue;
    const list = methodsOfOwner.get(owner);
    if (list) list.push(node);
    else methodsOfOwner.set(owner, [node]);
  }
...
function markupFileNode(file: MarkupFile, id: string): AtlasNode {
  const doc = file.doc;
  return {
    id,
    kind: 'file',
