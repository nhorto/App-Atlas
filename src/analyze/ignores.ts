...
export interface IgnoreMatcher {
  /** Every pattern in force — the defaults, and whatever the caller added. */
  readonly patterns: string[];
  /**
   * Whether a path on disk is out of view.
   *
   * True for a *directory* whose contents are ignored as well as for the files in it:
   * `examples/**` matches `examples` itself, which is what lets a walk prune a subtree
   * instead of descending into it and filtering on the way out.
   */
 ...
 */
export function buildIgnoreMatcher(root: string, extra: readonly string[] = []): IgnoreMatcher {
