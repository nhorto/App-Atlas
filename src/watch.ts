...
export interface WatchOptions {
  root: string;
  /** Milliseconds of quiet before a rebuild starts. */
  quietMs?: number;
  /**
   * Called with the repo-relative paths that changed. Awaited, and never called again
   * while it is still running — anything that happens meanwhile is folded into the
   * next call, so a save during a rebuild is noticed rather than dropped.
   */
...
export function watchProject(options: WatchOptions): Watcher {
  ...
  const watcher = fs.watch(root, { recursive: true }, (_event, filename) => {
    ...
    if (!isInteresting(relPath)) return;
    pending.add(relPath);
    schedule();
  });

  watcher.on('error', (err: Error) => options.onError?.(err));
...
export function isInteresting(relPath: string): boolean {
  ...
  for (let i = 0; i < segments.length - 1; i++) {
    if (IGNORED_DIRS.has(segments[i])) return false;
  }
