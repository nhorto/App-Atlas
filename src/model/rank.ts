...
export interface RankedFile {
  node: AtlasNode;
  /**
   * How many files this one imports directly — a fact the reader can go and check,
   * unlike the score, which is a number about a graph and means nothing on its own.
   *
   * It is deliberately not what the list is sorted by. A file that pulls in six modules
   * which are themselves the spine of the app outranks one that pulls in twenty leaves,
   * and that is the whole reason for ranking rather than counting.
   */
  imports: number;
}
...
export function rankFiles(nodes: Iterable<AtlasNode>, edges: Iterable<AtlasEdge>, limit = 10): RankedFile[] {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    let dangling = 0;
    for (const id of ids) if (!out.has(id)) dangling += score.get(id)!;
    ...
    let drift = 0;
    for (const id of ids) drift += Math.abs(next.get(id)! - score.get(id)!);
    score = next;
    if (drift < SETTLED) break;
  }
 ...
 */
function holdsSomething(node: AtlasNode): boolean {
  const meta = node.meta as { functionCount?: number; typeCount?: number };
