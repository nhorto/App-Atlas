...
export interface PackageIndex {
  /**
   * Which of `among` declares a dependency on `packageName`, in the order given.
   *
   * `among` is the set the caller is willing to name — in practice the packages this
   * project imports somewhere — so an answer is always a package the reader has a
   * relationship with, and the walk can never wander off into the install tree.
   */
  dependents(packageName: string, among: readonly string[]): string[];
}

/** An index that answers from nothing: what a caller with no `node_modules` gets. */
export const NO_PACKAGES: PackageIndex = { dependents: () => [] };
 ...
 */
export function installedPackages(root: string): PackageIndex {
  const manifests = new Map<string, Record<string, string>>();

  const declaredBy = (name: string): Record<string, string> => {
    ...
    try {
      ...
      if (!/^(?:@[\w.-]+\/)?[\w.-]+$/.test(name)) throw new Error('not a package name');
