...
import type { AtlasEdge, AtlasMeta, AtlasNode, EndpointMeta, SummarySource, Zone } from './types.js';

/** One file, and enough about it to decide whether to go and open it. */
export interface UnimportedFile {
  id: string;
  /** Repo-relative path — the whole of the claim, and how to check it. */
  path: string;
  zone: Zone;
  loc: number;
  /** The names it declares for others to use, which nobody is using. Capped. */
  exportedNames: string[];
  /** Its own docstring, when it has one. Says what it was *meant* to be. */
  summary: string | null;
 ...
 *
 * `answered: false` and an empty list are two different sentences, and a screen that
 * renders them the same way tells somebody their app is clean when nobody looked.
 */
export interface UnimportedView {
  answered: boolean;
  /** One sentence saying why nothing can be claimed. `null` once something can. */
  because: string | null;
  /**
   * The sentence to put in front of a reader, written here so the brief, the overview,
   * the MCP tool and anything after them cannot phrase the same repo differently — the
   * same reason `authHeadline` exists. Lower case and unpunctuated: each surface adds
   * its own capital and full stop. `null` when the question was refused, in which case
   * `because` is the sentence.
   */
  headline: string | null;
  /** The list, capped. `total` says how many there really were. */
  files: UnimportedFile[];
  total: number;
  /** Files weighed up — the denominator, so the number reads as a proportion. */
  considered: number;
  /** What this answer cannot see, in the order it should be read. Always non-empty. */
  caveats: string[];
}
 ...
 */
const ALWAYS =
  'a file reached by a computed path, by a name built from a string, or from a document App Atlas does not ' +
 ...
 */
export function findUnimported(nodes: AtlasNode[], edges: AtlasEdge[], meta: AtlasMeta): UnimportedView {
  ...
  const candidates: UnimportedFile[] = [];
  let considered = 0;
  let unread = 0;
 ...
 */
function headlineFor(found: number, considered: number): string {
  if (found > 0) {
    return found === 1
  ...
  if (considered === 0) {
    return (
      'there was nothing here to ask about: every file is a notebook, a test, a config file, a declared way ' +
      'in, or one a framework runs itself'
    );
  }
  return (
    `every one of the ${considered.toLocaleString('en-US')} source ${considered === 1 ? 'file' : 'files'} here ` +
 ...
 */
function refuseToAnswer(meta: AtlasMeta): string | null {
  if (!meta.coverage) {
    return (
      'this atlas was written by a version of App Atlas that did not record whether it traced who uses what, ' +
      'so there is no way to tell "nothing imports this" from "nobody looked". Re-run `app-atlas analyze`'
    );
  }
  if (!meta.coverage.references) {
    return (
      'the symbol-reference pass was skipped for this run (`--no-refs`), so App Atlas never traced who uses ' +
  ...
  if (unreadable.length > 0) {
    ...
    const named = unreadable
      ...
      .join(' and ');
    return (
      `this project contains ${named}, which App Atlas does not parse. Each one can import the code it ` +
      'renders, so an unknown number of links between files are missing from the graph'
    );
  }
  if (!meta.coverage.wholeRepo) {
    return (
      'this run mapped one app inside a larger repo, so the packages that import this one were never read. ' +
  ...
  if (meta.archetype?.archetype === 'library') {
    return (
      'this project is code other code imports, so its files are meant to be used from outside this repo — ' +
 ...
 */
function refuseOnBrokenLinks(nodes: AtlasNode[]): string | null {
  ...
  for (const node of nodes) {
    ...
    if (node.zone === 'test') continue;
    for (const specifier of (node.meta as { unresolvedImports?: string[] }).unresolvedImports ?? []) {
      specifiers.add(specifier);
      if (specifiers.size >= 3) break;
    }
  }
  if (specifiers.size === 0) return null;
  const shown = [...specifiers].sort().slice(0, 2).map((s) => `\`${s}\``).join(' and ');
  return (
    `App Atlas could not resolve import paths like ${shown} — they are aliases whose mapping is in a config ` +
 ...
 */
function filesInUse(nodes: AtlasNode[], edges: AtlasEdge[]): Set<string> {
  const used = new Set<string>();
  const fileOf = new Map<string, string>();
  for (const node of nodes) {
    if (node.path) fileOf.set(node.id, node.kind === 'file' ? node.id : makeFileId(node.path));
  }
  ...
  for (const node of nodes) {
    ...
    if (node.path) used.add(makeFileId(node.path));
    for (const site of (node.meta as unknown as EndpointMeta).sites ?? []) {
      if (site.path) used.add(makeFileId(site.path));
    }
  }
 ...
 */
function worthAsking(node: AtlasNode): boolean {
  if (node.zone === 'test' || node.zone === 'config') return false;
  const meta = node.meta as {
    ext?: string;
    exportedNames?: string[];
    frameworkOwned?: unknown;
    declaredEntry?: unknown;
    namespace?: unknown;
  };
  ...
  if ((meta.exportedNames ?? []).length === 0) return false;
  if (typeof meta.frameworkOwned === 'string') return false;
  if (typeof meta.declaredEntry === 'string') return false;
  ...
  if (meta.namespace === 'main') return false;
  const path = node.path ?? '';
  if (path.endsWith('.d.ts')) return false;
  if (NOT_IMPORTABLE.has(String(meta.ext ?? ''))) return false;
  ...
  if (/(^|\/)__[a-z_]+__\.pyi?$/.test(path)) return false;
  return true;
}

/** What the answer leaves out, in the order somebody should read it. */
function caveatsFor(found: number, unread: number, meta: AtlasMeta): string[] {
  const caveats = [ALWAYS];
  if (found > 1) {
    caveats.push(
      'a file that only the files above import is not itself listed — this is one step, not a sweep, ' +
        'so start at the top and work outwards',
    );
  }
  if (unread > 0) {
    caveats.push(
      `App Atlas could not read ${unread} ${unread === 1 ? 'file' : 'files'}; if one of them imports ` +
...
function describe(node: AtlasNode): UnimportedFile {
  const meta = node.meta as { loc?: number; exportedNames?: string[] };
  return {
    id: node.id,
