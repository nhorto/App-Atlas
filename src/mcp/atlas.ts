...
/** One analysed app this server can answer about. */
export interface AtlasApp {
  /** What `scope` has to be set to in order to ask about this one. Empty for a plain repo. */
  id: string;
  name: string;
  /** Absolute directory whose `.app-atlas/` holds the atlas. */
  dir: string;
}

/** An atlas that was found, or the reason there is nothing to answer from. */
export type Resolution =
  | { found: true; graph: AtlasGraph; app: AtlasApp }
...
export class AtlasSource {
   ...
   */
  resolve(scope?: string | null): Resolution {
    ...
    if (!graph) {
      return {
        found: false,
        because:
          `No atlas in ${app.dir}. This server reads an analysis that has already been run; ` +
