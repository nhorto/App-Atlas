...
> Generated by App Atlas v0.25.0 on 2026-08-13. Facts are derived by each language's own parser and type checker; sentences marked `(ai)` were generated and may be wrong. Re-run `app-atlas export` after the code changes.
...
- 196 files · 62,038 lines · 20 folders · 1,439 functions · 444 types
...
- **Browser storage** (localStorage) — 1 reads, 2 writes
...
Every one of the 111 source files here is imported by something else in the app, declared as a way in, or run by a framework convention.
...
- **AtlasGraph** (class, `src/model/graph.ts`) — used in 93 places — meta, rootId, nodes, children, relations, incoming, outgoing, pathCache, pathIndex
...
- **AtlasApp** (interface, `src/mcp/atlas.ts`) — used in 28 places — id, name, dir
...
- `web/src/App.tsx` (imports 16 files) — The app shell and its six lenses.
- `src/mcp/index.ts` (imports 4 files) — `app-atlas mcp` — the atlas, answered over the Model Context Protocol.
- `src/server/index.ts` (imports 15 files) — The local server.
