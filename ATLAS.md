...
> Generated by App Atlas v0.26.0 on 2026-08-13. Facts are derived by each language's own parser and type checker; sentences marked `(ai)` were generated and may be wrong. Re-run `app-atlas export` after the code changes.
...
- **Files on disk** (Node fs and Python) — 41 reads, 17 writes
...
Every one of the 112 source files here is imported by something else in the app, declared as a way in, or run by a framework convention.
...
- `src/analyze/index.ts` (imports 24 files) — Analysis orchestrator.
