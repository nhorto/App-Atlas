...
import { claimStdout, LineFramer, parseMessage, RPC_ERROR, rpcError, rpcResult, } from './protocol.js';
...
export { claimStdout, encodeMessage, LineFramer, parseMessage, RPC_ERROR, } from './protocol.js';
/**
 ...
 */
const INSTRUCTIONS = 'App Atlas maps this codebase from its source: every way in, what guards each one, where data goes, and what ' +
 ...
 */
export function handleMcpMessage(source, request) {
    ...
    switch (request.method) {
        case 'initialize': {
            ...
            return rpcResult(id, {
                protocolVersion: typeof requested === 'string' && SUPPORTED_PROTOCOLS.includes(requested)
                    ? requested
    ...
    try {
        ...
        for await (const chunk of process.stdin) {
            for (const line of framer.push(String(chunk)))
                answerLine(source, line, stream);
        }
        const trailing = framer.rest();
        if (trailing)
            answerLine(source, trailing, stream);
    }
    finally {
        stream.release();
    }
 ...
 */
function answerLine(source, line, stream) {
    ...
    try {
        ...
        if ('id' in incoming.request) {
            stream.send(rpcError(incoming.request.id ?? null, RPC_ERROR.internal, err.message));
        }
    }
}
