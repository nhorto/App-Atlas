 ...
 */
function findGitDirs(from) {
    let dir = from;
    for (;;) {
        const candidate = path.join(dir, '.git');
        const stat = statOrNull(candidate);
        if (stat?.isDirectory())
            return { git: candidate, common: candidate };
        if (stat?.isFile()) {
...
function packedRef(common, name) {
