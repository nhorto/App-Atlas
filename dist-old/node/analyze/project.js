 ...
 */
const UNREAD_LANGUAGE_GLOB = '**/*.{rb,php,java,kt,kts,scala,groovy,ex,exs,erl,swift,m,mm,dart,clj,cljs,hs,lua,pl,pm,jl,zig,nim,cr,fs,vb}';
/**
 ...
 */
const TEMPLATE_GLOB = '**/{templates,template,views,jinja,jinja2}/**/*.{html,htm,jinja,jinja2,j2,twig,erb,hbs,handlebars,mustache,ejs,liquid}';
...
export async function discoverProject(rootInput, options) {
    ...
    const name = (typeof packageJson?.name === 'string' && packageJson.name.trim()) || path.basename(root) || 'app';
    ...
    const deploymentRoot = options.inheritDeploymentFiles !== false && options.repoRoot ? path.resolve(options.repoRoot) : root;
    ...
    if (relPaths.length > options.maxFiles) {
        warnings.push(`Found ${relPaths.length} source files; analyzing the first ${options.maxFiles}. Raise the cap with --max-files.`);
...
function countByExtension(relPaths) {
...
function readWorkspaces(root, pkg) {
    const out = [];
...
function detectFrameworks(pkg, signals) {
    const out = new Set();
    if (pkg) {
        const deps = {
            ...pkg.dependencies,
    ...
    if (signals.nextAppDir)
        out.add('Next.js App Router');
    if (signals.nextPagesDir && !signals.nextAppDir)
        out.add('Next.js Pages Router');
    if (signals.expoRouterDir)
        out.add('Expo Router');
    if (signals.crons.length > 0)
        out.add('Vercel Cron');
    ...
    if (signals.workers.some(isWorker))
        out.add('Cloudflare Workers');
    if (signals.workers.some((w) => w.isPages))
        out.add('Cloudflare Pages');
    for (const [dep, label] of Object.entries(PYTHON_FRAMEWORKS)) {
        if (signals.pythonPackages.has(dep))
            out.add(label);
    }
 ...
 */
function applyGitignore(root, relPaths) {
