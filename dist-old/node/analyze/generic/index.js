 ...
 */
export const genericPlugins = LANGUAGES.map((language) => ({
    id: language.dialect.id,
    displayName: language.dialect.displayName,
    claims: (file) => languageFor(file.relPath)?.dialect.id === language.dialect.id,
    analyze: (ctx) => analyzeGeneric(language, ctx),
}));
export async function analyzeGeneric(language, ctx) {
    ...
    for (const ref of files) {
        ...
        if (typeof namespace === 'string')
            namespaceOf.set(ref.relPath, namespace);
        reused++;
    }
    if (stale.length === 0) {
    ...
    try {
        ...
        warnings.push(`Found ${files.length} ${dialect.displayName} ${files.length === 1 ? 'file' : 'files'} but could not start the parser: ` +
            `${err.message}. They appear on the map without their insides.`);
        for (const ref of stale)
            nodes.push(shallowFileNode(ref, project.root, dialect.id, 'the tree-sitter grammar for it would not load'));
    ...
    for (const ref of stale) {
        ...
        if (read.namespace)
            namespaceOf.set(ref.relPath, read.namespace);
    ...
    for (const ref of stale) {
        ...
        const bucket = { nodes: [], edges: new Map(), boundaries: [] };
    ...
    for (const [relPath, namespace] of namespaceOf) {
        ...
        if (list)
            ...
            byNamespace.set(namespace, [relPath]);
    }
    const scope = {
    ...
    for (const ref of stale) {
        ...
        if (options.detectBoundaries && language.boundaries) {
            ...
            bucket.boundaries.push(...language.boundaries({
                file,
                fileId,
 ...
 * to the wrong half.
 */
function mergePartialTypes(nodes, edges, boundaries, namespaceOf) {
    const groups = new Map();
    for (const node of nodes) {
        if (node.kind !== 'type' || node.meta.partialType !== true)
            continue;
    ...
    for (const parts of groups.values()) {
        ...
        parts.sort((a, b) => canonical(a) - canonical(b) ||
        ...
        for (const part of parts) {
            ...
            for (const base of extendsOf(part))
                bases.add(base);
    ...
    for (const node of out) {
        if (node.parentId && replaced.has(node.parentId))
            node.parentId = replaced.get(node.parentId);
    }
    for (const [id, edge] of [...edges]) {
        const fromId = replaced.get(edge.fromId) ?? edge.fromId;
        const toId = replaced.get(edge.toId) ?? edge.toId;
        if (fromId === edge.fromId && toId === edge.toId)
            continue;
        edges.delete(id);
        ...
        if (!edges.has(newId))
            edges.set(newId, { ...edge, id: newId, fromId, toId });
    }
    ...
    for (const finding of boundaries) {
        ...
        for (const key of ['nodeId', 'handlerId', 'sourceId']) {
            ...
            if (typeof value === 'string' && replaced.has(value))
                record[key] = replaced.get(value);
        }
        const site = record.site;
        if (site?.nodeId && replaced.has(site.nodeId))
            site.nodeId = replaced.get(site.nodeId);
    }
    return out;
}
/**
 ...
 */
function synthesizeRouteHandlers(bucket, ref, language) {
    const used = new Set(bucket.nodes.map((node) => node.id));
    const spans = [];
    for (const finding of bucket.boundaries) {
        if (finding.type !== 'endpoint' || !finding.handlerSpan)
            continue;
    ...
    const owner = (line) => {
        ...
        for (const span of spans) {
            if (line < span.line || line > span.endLine)
                continue;
            if (!best || span.endLine - span.line < best.endLine - best.line)
                best = span;
        }
        return best?.id ?? null;
    };
...
function fieldsOf(node) {
    return Array.isArray(node.meta.fields) ? node.meta.fields : [];
}
function extendsOf(node) {
    return Array.isArray(node.meta.extends) ? node.meta.extends : [];
}
// ---------------------------------------------------------------------------
// Nodes
// ---------------------------------------------------------------------------
...
function buildNodes(ref, file, text, ownModule, dialect) {
    const fileId = makeFileId(ref.relPath);
    const nodes = [];
    const used = new Set();
...
function functionNode(id, parentId, ref, def, language) {
...
function typeNode(id, fileId, ref, def, language) {
    const fields = def.fields.map((f) => ({ name: f.name, type: f.type || 'any', optional: false }));
    const doc = firstSentence(def.doc);
    return {
        id,
        kind: 'type',
 ...
 */
function shallowFileNode(ref, root, language, because) {
...
export function buildPackageIndex(relPaths) {
    const byDir = new Map();
 ...
 */
export function resolveImport(index, module, ownModule) {
    if (ownModule) {
        if (module === ownModule)
            return index.byDir.has('') ? '' : null;
 ...
 */
function enclosingNamespaces(namespace) {
...
function linkFile(ref, file, index, declarations, edges, ownModule, scope) {
    const fileId = makeFileId(ref.relPath);
    ...
    const filesOf = (group) => byNamespace ? (scope.byNamespace.get(group) ?? []) : (index.byDir.get(group) ?? []);
    /**
     ...
     */
    const mentioned = new Set(file.uses);
    for (const def of file.defs) {
    ...
    const itemImports = new Map();
    for (const imp of file.imports) {
        let group;
        let item = null;
        if (byNamespace) {
            if (scope.byNamespace.has(imp.module))
                group = imp.module;
                ...
                if (group)
                    item = imp.module.slice(dot + 1);
            }
            else
                group = null;
        }
        else
            group = resolveImport(index, imp.module, ownModule);
        if (group === null)
            continue;
        packageDirs.set(imp.local, group);
        if (scope.moduleIsAFile)
            itemImports.set(imp.local, { group, item: item ?? imp.local });
        for (const target of filesOf(group)) {
            if (target === ref.relPath)
                continue;
    ...
    for (const group of visible) {
        for (const sibling of filesOf(group)) {
            for (const [name, decl] of declarations.get(sibling) ?? []) {
                if (!inPackage.has(name))
                    inPackage.set(name, decl);
            }
        }
    }
    const declarationsIn = (group, name) => {
...
function addEdge(edges, input) {
    ...
    if (existing) {
        ...
        const added = input.meta.symbols ?? [];
        if (added.length > 0) {
            const already = existing.meta.symbols ?? [];
 ...
 */
function externalImports(file, ownModule, dialect) {
    const out = new Set();
    for (const imp of file.imports) {
        if (dialect.externalImport) {
            if (dialect.externalImport(imp.module))
                out.add(imp.module);
            continue;
        }
        ...
        if (!first.includes('.'))
            continue;
        if (ownModule && (imp.module === ownModule || imp.module.startsWith(`${ownModule}/`)))
            continue;
        out.add(imp.module);
    }
    return [...out].sort();
