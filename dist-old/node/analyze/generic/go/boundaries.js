...
export function detectGoBoundaries(input) {
    const { file } = input;
    const findings = [];
    ...
    const importsModule = (prefix) => file.imports.some((imp) => imp.module === prefix || imp.module.startsWith(`${prefix}/`));
...
function detectSignInCalls(input, importsModule, findings, at) {
    for (const call of input.file.calls) {
        if (!call.method || !call.receiver || !call.scope)
            continue;
...
function detectRoutes(input, imports, findings, at) {
    const { file } = input;
    ...
    for (const def of file.defs) {
        for (const param of def.params) {
            ...
            if (!CALLER_PREFIXED_TYPE.test(type))
                continue;
            ...
            if (def.kind !== 'function' || named.has(def.name))
                continue;
            named.add(def.name);
        ...
        for (const def of file.defs) {
            for (const param of def.params) {
                ...
                for (const { field, framework } of fields) {
                    ...
                    if (!routers.has(held))
                        routers.set(held, framework);
                }
            }
        }
    }
    const frameworkOfPackage = (local) => {
     ...
     */
    const enclosingDef = (call) => {
        let best = null;
        for (const def of file.defs) {
            if (def.kind !== 'function')
                continue;
            if (def.startIndex > call.startIndex || def.endIndex < call.endIndex)
                continue;
            if (!best || def.startIndex > best.startIndex)
                best = def;
        }
        return best;
    };
     ...
     */
    const wrappedFramework = (call, handed) => {
        if (!handed)
            return null;
        const dot = handed.type.indexOf('.');
        if (dot <= 0 || call.receiver !== handed.type.slice(0, dot))
            return null;
     ...
     */
    const enclosingCall = (call) => {
        let best = null;
        for (const other of file.calls) {
            if (other.startIndex === call.startIndex)
                continue;
            if (other.startIndex > call.startIndex || other.endIndex < call.endIndex)
                continue;
            if (!best || other.startIndex > best.startIndex)
                best = other;
        }
        return best;
    };
    /** What a router variable is called outside the function it was built in. */
    const identityOf = (varName, index) => {
        for (const alias of aliases) {
            if (alias.varName !== varName)
                continue;
            if (alias.startIndex <= index && alias.endIndex >= index)
    ...
    const innermost = (index) => {
        ...
        for (const scope of scopes) {
            if (scope.startIndex > index || scope.endIndex < index)
                continue;
            if (!best || scope.startIndex > best.startIndex)
                best = scope;
        }
        return best;
    };
    ...
    for (const call of byPosition) {
        ...
        if (!host || !SUBROUTERS.has(method))
            continue;
        const pattern = patternOf(call.args);
        const closure = call.args.find((arg) => arg.t === 'func');
        ...
        if (closure)
            scopes.push({ varName, startIndex: closure.startIndex, endIndex: closure.endIndex });
        findings.push({
            type: 'router-build',
            routerName: call.callee,
            varName,
        ...
        const attached = args.map(nameOf).filter((name) => name !== null);
        for (const chained of byPosition) {
            if (chained.startIndex !== call.startIndex || !ATTACHES.has(chained.method ?? ''))
                continue;
            for (const name of chained.args.map(nameOf))
                if (name)
                    attached.push(name);
        }
    ...
    for (const call of byPosition) {
        ...
        if (method === 'Use') {
            const names = call.args.map(nameOf).filter((name) => name !== null);
            if (names.length > 0) {
        ...
        if (method === 'Mount') {
            ...
            const named = call.args.map(nameOf).filter((name) => name !== null);
        ...
        const after = call.args.slice(1).map(nameOf).filter((name) => name !== null);
        ...
        if (call.scope)
            wiring.add(call.scope.split('.').pop() ?? call.scope);
    ...
    for (const call of byPosition) {
        if (!ATTACHES.has(call.method ?? ''))
            continue;
        if (!call.receiver?.includes('('))
            continue;
        const door = doorsAt.get(call.startIndex);
        if (!door || door.site.line !== call.line)
            continue;
        for (const name of call.args.map(nameOf)) {
            if (name)
                door.paramTypes?.push(name.split('.').pop() ?? name);
        }
    }
    return wiring;
}
 ...
 */
function patternOf(args) {
    const first = args[0];
    if (first?.t === 'str')
        return { prefix: first.v, prefixName: null };
    if (first?.t === 'name')
        return { prefix: null, prefixName: first.v };
 ...
 */
function detectPathConstants(file, findings) {
    for (const constant of file.constants) {
        if (constant.value !== '' && !constant.value.startsWith('/'))
            continue;
        findings.push({
            type: 'path-constant',
            name: constant.name,
 ...
 */
function packageDir(module, ownModule) {
    if (!ownModule || !module.startsWith(`${ownModule}/`))
        return module;
 ...
 */
function boundName(file, call) {
    for (const binding of file.bindings) {
        if (binding.line !== call.line)
            continue;
        if (binding.callee === call.callee || binding.callee.startsWith(`${call.callee}(`))
            return binding.name;
    }
    return null;
}
 ...
 */
function routeFor(call, method, framework) {
    ...
    if (method === 'Method' || method === 'MethodFunc') {
        ...
        if (!METHODS.has(verb) || route === null)
            return null;
    ...
    if (split && METHODS.has(split[1]))
        return { method: split[1], route: split[2] };
 ...
 * the two.
 */
function detectCheckers(input, wiring, findings) {
    const { file } = input;
 ...
 */
function rejectingFunctions(file, wiring) {
    ...
    for (const def of file.defs) {
        ...
        if (def.kind !== 'function' || wiring.has(def.name))
            continue;
        if (!declared.has(def.name))
            declared.set(def.name, def);
    }
    const rejecting = new Set();
    for (const [name, def] of declared) {
        if (writesARejection(def, file))
            rejecting.add(name);
    }
    for (let hop = 0; hop < MAX_REJECT_HOPS; hop++) {
        ...
        for (const [name, def] of declared) {
            if (rejecting.has(name))
                continue;
            if (!def.uses.some((used) => rejecting.has(used)))
                continue;
            rejecting.add(name);
            grew = true;
        }
...
// Configuration
// ---------------------------------------------------------------------------
function detectEnv(file, imports, findings, at) {
    if (imports.get('os') !== 'os')
        return;
    for (const call of file.calls) {
        if (call.receiver !== 'os')
            continue;
        if (call.method !== 'Getenv' && call.method !== 'LookupEnv')
            continue;
        const name = firstString(call.args);
        if (name)
            findings.push({ type: 'env', name, site: at(call, `os.${call.method}("${name}")`) });
    }
}
const STORE_RULES = [
    {
 ...
 */
function detectStores(file, importsModule, findings, at) {
    ...
    for (const call of file.calls) {
        const method = call.method ?? '';
        if (!call.receiver || claimed.has(call.line))
            continue;
 ...
 */
function detectOutbound(file, imports, findings, at) {
    if (imports.get('http') !== 'net/http')
        return;
    for (const call of file.calls) {
        ...
        if (!['Get', 'Post', 'PostForm', 'Head'].includes(method) && !isRequest)
            continue;
