...
export const jobsDetector = {
    id: 'jobs',
    enabled: () => true,
    visit(node, ctx) {
        if (Node.isNewExpression(node))
            return newExpression(node, ctx);
        if (!Node.isCallExpression(node)) {
            ...
            if (Node.isPropertyAccessExpression(node) && node.getExpression().getText() === 'process') {
                if (node.getName() === 'argv')
                    emitCli(node, ctx, 'process.argv');
            }
            return;
        }
...
function callExpression(call, ctx) {
    ...
    if (dotted === 'createFunction' || dotted.endsWith('.createFunction')) {
        if (!ctx.signals.packages.has('inngest'))
            return;
        const id = literalString(objectProp(argAt(call, 0), 'id')) ?? 'function';
        const trigger = literalString(objectProp(argAt(call, 1), 'event')) ?? literalString(objectProp(argAt(call, 1), 'cron'));
    ...
    if (dotted === 'task' && ctx.signals.packages.has('@trigger.dev/sdk')) {
        ...
        if (id)
            return emitQueue(ctx, call, id, 'Trigger.dev');
    }
    ...
    if ((last === 'handle' || last === 'on' || last === 'handleOnce' || last === 'once') && root === 'ipcMain') {
        ...
        if (channel)
            return emitIpc(ctx, call, channel, dotted);
        return;
    }
    ...
    if (last === 'on' && (root === 'io' || root === 'wss' || root === 'socket')) {
        ...
        if (event === 'connection' || event === 'connect')
            return emitRealtime(ctx, call, event, 'WebSocket');
    }
...
function newExpression(node, ctx) {
...
// ---------------------------------------------------------------------------
function emitCron(ctx, at, expression, framework) {
...
function emitQueue(ctx, at, name, framework) {
    ctx.emit({
        type: 'endpoint',
        endpointKind: 'queue',
        key: `queue ${name}`,
...
function emitRealtime(ctx, at, name, framework) {
    ctx.emit({
        type: 'endpoint',
 ...
 */
function bridgedCapabilities(ctx, call) {
    const api = literalString(argAt(call, 0));
    const surface = argAt(call, 1);
    if (!api || !surface || !Node.isObjectLiteralExpression(surface))
        return;
    for (const member of surface.getProperties()) {
        if (!Node.isPropertyAssignment(member) && !Node.isMethodDeclaration(member))
            continue;
        ...
        body.forEachDescendant((child) => {
            if (reaches || !Node.isCallExpression(child))
                return;
            ...
            if (PRIVILEGED_MODULES.test(parts[0]) && parts.length > 1)
                reaches = inner;
        });
        if (reaches === null || reaches === 'ipc')
            continue;
        ctx.emit({
            type: 'endpoint',
            endpointKind: 'ipc',
...
function emitIpc(ctx, at, channel, dotted) {
    ctx.emit({
        type: 'endpoint',
        endpointKind: 'ipc',
        key: `ipc ${channel}`,
...
function emitCli(at, ctx, snippet) {
    ctx.emit({
        type: 'endpoint',
        endpointKind: 'cli',
        key: 'cli',
...
function isFrom(local, ctx, pkg) {
    const binding = ctx.imports.get(local);
    if (binding?.external && binding.module === pkg)
        return true;
