 ...
 * others.
 */
export function readComposePorts(repoRoot, appRoot = repoRoot, ignores = buildIgnoreMatcher(appRoot), maxDepth = 3) {
    const out = [];
 ...
 */
function isTestScaffolding(repoRoot, absPath) {
    return classifyZone(toPosix(path.relative(repoRoot, absPath))) === 'test';
}
function readOne(appRoot, absPath) {
    let text;
    try {
...
function spec(bind, host, container, protocol) {
    if (!PORT_NUMBER.test(container) && !container.includes('$'))
        return null;
    let hostPort = null;
    let hostPortVar = null;
...
function resolve(written) {
    return written.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)(?::?([-?+])([^}]*))?\}/g, (_all, name, op, fallback) => op === '-' ? (fallback ?? '') : `$${name}`);
}
/**
...
function dropBlockScalars(lines) {
    const out = [];
    for (let i = 0; i < lines.length; i++) {
        out.push(lines[i]);
        if (!BLOCK_SCALAR.test(lines[i].text))
            continue;
        while (i + 1 < lines.length && lines[i + 1].indent > lines[i].indent)
            i++;
    }
    return out;
}
...
function childKeys(lines, parent) {
    ...
    for (let i = start; i < end; i++)
        column = Math.min(column, lines[i].indent);
    const keys = [];
    for (let i = start; i < end; i++)
        if (lines[i].indent === column)
            keys.push(i);
    return keys;
}
/** Where the block a key opens starts and stops. */
function childRange(lines, parent) {
    const start = parent + 1;
    let end = start;
    while (end < lines.length && lines[end].indent > lines[parent].indent)
        end++;
    return [start, end];
}
 ...
 */
function sequenceItems(lines, keyAt, inlineValue) {
    ...
    for (let i = start; i < end; i++) {
        const line = lines[i];
        if (line.indent !== column || !line.text.startsWith('-'))
            continue;
        ...
        if (/^[A-Za-z_][A-Za-z0-9_-]*:(\s|$)/.test(rest)) {
            ...
            if (first)
                map.set(first.key, resolve(unquote(first.value)));
            let j = i + 1;
            for (; j < end && lines[j].indent > column; j++) {
                const pair = splitPair(lines[j]);
                if (pair)
                    map.set(pair.key, resolve(unquote(pair.value)));
            }
