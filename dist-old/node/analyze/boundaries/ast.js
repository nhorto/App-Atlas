...
export function isRouterCallee(callee) {
 ...
 */
export function looksLikeRouter(name, locals) {
    if (ROUTER_NAMES.test(name))
        return true;
 ...
 */
export function literalPrefix(node) {
...
export function argAt(call, index) {
 ...
 */
export function constantString(node, depth = 0) {
    if (!node || depth > MAX_CONST_HOPS)
        return null;
...
function objectBehind(node, depth) {
    if (Node.isObjectLiteralExpression(node))
        return node;
    if (Node.isAsExpression(node) || Node.isParenthesizedExpression(node)) {
        return objectBehind(node.getExpression(), depth);
    }
    if (depth >= MAX_CONST_HOPS)
        return null;
...
export function functionArg(call) {
    const args = call.getArguments();
    for (let i = args.length - 1; i >= 0; i--) {
        const arg = args[i];
        if (Node.isArrowFunction(arg) || Node.isFunctionExpression(arg) || Node.isIdentifier(arg))
            return arg;
    }
    return undefined;
}
...
export function snippetOf(node, max = 120) {
 ...
 */
export function permitsEverything(arg) {
    ...
    for (const declaration of declarations) {
        ...
        if (!Node.isReturnStatement(only))
            continue;
        if (only.getExpression()?.getKind() === SyntaxKind.TrueKeyword)
            return true;
    }
    return false;
}
 ...
 */
export function alwaysContinues(fn) {
    ...
    if (!body || !Node.isBlock(body))
        return false;
    const mine = (node) => enclosingFunctionOf(node) === fn;
    const mentionsNext = (node) => node.getDescendantsOfKind(SyntaxKind.Identifier).some((id) => id.getText() === next);
    for (const ret of body.getDescendantsOfKind(SyntaxKind.ReturnStatement)) {
        if (mine(ret) && !mentionsNext(ret))
            return false;
    }
    for (const thrown of body.getDescendantsOfKind(SyntaxKind.ThrowStatement)) {
        if (mine(thrown) && !thrown.getFirstAncestorByKind(SyntaxKind.CatchClause))
            return false;
    }
 ...
 */
export function enclosingFunctionOf(node) {
    return node.getFirstAncestor((a) => Node.isFunctionDeclaration(a) ||
 ...
 */
export function functionBehind(node, depth = 0) {
    if (depth > 3)
        return null;
    ...
    for (const declaration of (aliased ?? symbol).getDeclarations() ?? []) {
        ...
        const initializer = Node.isVariableDeclaration(declaration) || Node.isPropertyAssignment(declaration)
