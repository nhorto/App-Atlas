 ...
 */
import { unreadFromSite } from './address.js';
const GAP = { known: null };
 ...
 */
export function composeRoutePrefixes(findings) {
    const builds = [];
    const mounts = [];
    const constants = [];
    const globals = [];
    ...
    for (const finding of findings) {
        if (finding.type === 'router-build')
            builds.push(finding);
        else if (finding.type === 'router-mount')
            candidates.push(finding);
        else if (finding.type === 'path-constant')
            constants.push(finding);
        else if (finding.type === 'global-prefix')
            globals.push(finding);
        else if (finding.type === 'mount-method')
            wrappers.add(finding.name);
    }
    ...
    for (const mount of candidates) {
        ...
        if (method === undefined || method === 'use' || method === 'route' || wrappers.has(method))
            mounts.push(mount);
    }
    const hasPrefixes = mounts.length > 0 || globals.length > 0 || builds.some((build) => build.hasPrefix);
    // A repo can declare doors whose head is unread and mount nothing at all — a Strapi
    // plugin is a directory of route data and no `use` call anywhere — so the ellipsis
    // pass has to run even when there is no prefix in the repo to compose.
    const hasUnplaced = findings.some((finding) => finding.type === 'endpoint' && (finding.prefixFromCaller || finding.prefixUnread));
    if (!hasPrefixes && !hasUnplaced)
        return findings;
    const chains = new Chains(builds, mounts, constants);
    const everywhere = globalPrefixes(globals);
    return findings.flatMap((finding) => {
        if (finding.type === 'guard')
            return placeMatchers(finding, chains);
        ...
        if (finding.type !== 'endpoint' || finding.route === null)
            return finding;
        // The detector already knows the head is not written anywhere in this repo — a
        // Strapi plugin's prefix is its *directory name*, read off a registry at boot. No
        // mount can supply what was never written, so composition is not attempted rather
        // than attempted and failed: trying would let an unrelated mount in the same package
        // put a prefix in front of it, which is how a door gets an address nobody serves it
        // at. `prefixFromCaller` is the opposite case and still goes the long way round —
        // there the head does exist somewhere and is worth looking for.
        if (finding.prefixUnread)
            return unreadFromSite(finding);
 ...
 */
function globalPrefixes(findings) {
    const byFramework = new Map();
    for (const finding of findings) {
        const values = byFramework.get(finding.framework);
        if (values)
            values.add(finding.prefix);
        else
            byFramework.set(finding.framework, new Set([finding.prefix]));
    }
    const out = new Map();
...
function applyPrefix(finding, parts) {
    const route = render(parts, finding.route);
    if (route === finding.route)
        return finding;
    return {
        ...finding,
        route,
...
function render(parts, route) {
    const pieces = [];
    for (const part of parts) {
        const text = part.known === null ? ELLIPSIS : trimPath(part.known);
        if (!text)
            continue;
        if (text === ELLIPSIS && pieces[pieces.length - 1] === ELLIPSIS)
            continue;
        pieces.push(text);
    }
 ...
 */
export function answersTo(where, wanted) {
 ...
 */
export function routerKey(module, varName) {
...
class Builds {
    ...
    constructor(builds) {
        for (const build of builds) {
            ...
            if (list)
                ...
                this.byVar.set(build.varName, [build]);
        }
    }
    /**
    ...
    childOf(mount) {
        ...
        if (wanted === '')
            return null;
        return (this.only(mount.childVar, (build) => answersTo(moduleOf(build.path), wanted)) ??
     ...
     */
    only(childVar, matches) {
        const byName = childVar ? (this.byVar.get(childVar) ?? []).filter(matches) : [];
        if (byName.length === 1)
            return routerKey(moduleOf(byName[0].path), byName[0].varName);
...
export function mountGraph(builds, mounts) {
...
class Chains {
    ...
    memo = new Map();
    constructor(builds, mounts, constants) {
        this.index = new Builds(builds);
        this.mountedAt = mountGraph(builds, mounts);
        for (const mount of mounts)
            this.hosts.add(routerKey(moduleOf(mount.path), mount.hostVar));
        for (const constant of constants) {
            const values = this.constants.get(constant.name);
            if (values)
                values.add(constant.value);
            else
                this.constants.set(constant.name, new Set([constant.value]));
    ...
    addressesFor(key) {
        ...
        if (distinct.size === 1)
            return [options[0]];
        if (distinct.size > MAX_ADDRESSES)
            return [this.prefixFor(key)];
        const named = [...distinct.values()];
        if (named.some((parts) => parts.some((part) => part.known === null)))
            return [this.prefixFor(key)];
        return named;
    }
    ...
    mountPrefix(mount) {
        ...
        if (mount.prefixOnlyIfNamed && part.known === null)
            return [];
        return [part];
    }
    /**
