...
function prisma(call, dotted, ctx) {
    if (!ctx.signals.packages.has('@prisma/client'))
        return false;
    ...
    const root = parts[0];
    if (!PRISMA_READS.has(operation) && !PRISMA_WRITES.has(operation))
        return false;
    if (!isPrismaClient(root, ctx))
        return false;
...
function isPrismaClient(root, ctx) {
    const local = ctx.locals.get(root);
    if (local && /PrismaClient/.test(local.callee))
        return true;
...
function drizzle(call, dotted, ctx) {
    if (!ctx.signals.packages.has('drizzle-orm'))
        return false;
    const parts = dotted.split('.');
    const operation = parts[parts.length - 1];
    const root = parts[0];
    if (!/^(db|database|drizzle|conn)$/i.test(root) && !isBuiltBy(root, ctx, 'drizzle'))
        return false;
...
function kysely(call, dotted, ctx) {
    const parts = dotted.split('.');
    const operation = parts[parts.length - 1];
    const root = parts[0];
...
function supabase(call, dotted, ctx) {
    if (!ctx.signals.packages.has('@supabase/supabase-js'))
        return false;
    const parts = dotted.split('.');
    const last = parts[parts.length - 1];
    const root = parts[0];
    if (!/^(supabase|sb|client|supabaseAdmin)$/i.test(root) && !isBuiltBy(root, ctx, 'createClient'))
        return false;
...
// Mongoose / raw SQL
// ---------------------------------------------------------------------------
function mongoose(call, dotted, ctx) {
    if (!ctx.signals.packages.has('mongoose') && !ctx.signals.packages.has('mongodb'))
        return false;
    const parts = dotted.split('.');
    const last = parts[parts.length - 1];
    ...
    if (!reads.has(last) && !writes.has(last))
        return false;
...
function rawSql(call, dotted, ctx) {
    const parts = dotted.split('.');
    const last = parts[parts.length - 1];
    if (last !== 'query' && last !== 'execute' && last !== 'unsafe')
        return false;
    const sqlPackage = ['pg', 'postgres', 'mysql2', '@neondatabase/serverless', '@vercel/postgres', 'better-sqlite3'].find((pkg) => ctx.signals.packages.has(pkg));
    if (!sqlPackage)
        return false;
...
function templateSql(call) {
    const arg = call.getArguments()[0];
    if (arg && Node.isTemplateExpression(arg))
        return arg.getText();
    return null;
}
// ---------------------------------------------------------------------------
 ...
 */
function keyValue(call, dotted, ctx) {
    const parts = dotted.split('.');
    ...
    if (!reads && !writes)
        return false;
    const root = parts[0];
    const pkg = ['@vercel/kv', '@upstash/redis', 'ioredis', 'redis'].find((name) => ctx.signals.packages.has(name));
    if (!pkg)
        return false;
    ...
    const known = imported?.module === pkg ||
...
// ---------------------------------------------------------------------------
function s3Client(node, ctx) {
    const name = dottedName(node.getExpression());
    if (name !== 'S3Client' && name !== 'S3')
        return;
    if (!ctx.signals.packages.has('@aws-sdk/client-s3') && !ctx.signals.packages.has('aws-sdk'))
        return;
    emit(ctx, node, {
        key: 's3',
        name: 'Amazon S3',
...
function blobWrite(call, dotted, ctx) {
 ...
 */
function browserStorage(call, dotted, ctx) {
    const parts = dotted.split('.');
    const operation = parts[parts.length - 1];
...
function fileAccess(call, dotted, ctx) {
    const parts = dotted.split('.');
    const last = parts[parts.length - 1];
    const writes = FS_WRITES.has(last);
    if (!writes && !FS_READS.has(last))
        return false;
    const root = parts[0];
...
function isBuiltBy(local, ctx, callee) {
...
function tableFromArgOrChain(call, operation) {
...
function chainedCallNames(call) {
    const names = [];
