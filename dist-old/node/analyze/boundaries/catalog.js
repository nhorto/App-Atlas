 ...
 */
const ENTRY_BY_METHOD = new Map();
for (const { methods, namespace, entry } of AUTH_ENTRY_CALLS) {
    for (const method of methods) {
        const rows = ENTRY_BY_METHOD.get(method);
        if (rows)
            rows.push({ namespace, entry });
        else
            ENTRY_BY_METHOD.set(method, [{ namespace, entry }]);
    }
}
/**
 ...
 */
export function authEntryForCall(dotted, rootProvider) {
    ...
    if (!imported && !rows)
        return null;
    if (PRIVILEGED_SEGMENT.test(dotted))
        return null;
    if (imported && rootProvider)
        return { provider: rootProvider, what: imported };
    const receiver = dot === -1 ? '' : dotted.slice(0, dot);
    for (const row of rows ?? []) {
        if (row.namespace.test(receiver))
            return row.entry;
    }
    return null;
}
export function serviceForPackage(pkg) {
    if (IN_PROCESS_AUTH.has(pkg))
        return null;
 ...
 */
export function knownServiceNames() {
    const names = new Set();
    for (const def of Object.values(PACKAGE_SERVICES))
        names.add(def.name);
    for (const def of Object.values(PYTHON_SERVICES))
        names.add(def.name);
    for (const [, def] of PYTHON_MODULE_SERVICES)
        names.add(def.name);
    for (const { def } of HOST_SERVICES)
        names.add(def.name);
    return [...names].sort();
}
...
export function storeForPackage(pkg) {
...
export function serviceForPythonModule(module) {
    let best = null;
    let longest = 0;
...
export function storeForPythonModule(module) {
...
export function prismaProviderName(provider) {
    switch (provider) {
        case 'postgresql':
        case 'postgres':
            return 'PostgreSQL';
