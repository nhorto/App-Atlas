 ...
 */
export async function findScopes(root) {
 ...
 */
export async function findWorkspace(root) {
    const byDir = new Map();
    const globs = workspaceGlobs(root);
    if (globs.length > 0) {
        const found = await fg(globs.map((glob) => `${glob.replace(/\/$/, '')}/{package.json,pyproject.toml}`), {
            cwd: root,
            dot: false,
            onlyFiles: true,
    ...
    for (const scope of dotnetScopes(root)) {
        if (!byDir.has(scope.dir))
            byDir.set(scope.dir, scope);
    }
 ...
 */
function includeTheRootWhenItIsTheProject(root, scopes, files) {
 ...
 */
function leadWithTheMainApp(scopes, files) {
    let best = -1;
    for (let i = 0; i < scopes.length; i++) {
        if (scopes[i].kind !== 'app')
            continue;
        if (best === -1 || (files.get(scopes[i].dir) ?? 0) > (files.get(scopes[best].dir) ?? 0))
            best = i;
    }
    if (best <= 0)
        return scopes;
 ...
 */
async function measure(root, scopes) {
    const dirs = [...scopes].sort((a, b) => b.dir.length - a.dir.length);
    const counts = new Map(dirs.map((scope) => [scope.dir, 0]));
    counts.set(ROOT, 0);
...
function workspaceGlobs(root) {
    ...
    if (fs.existsSync(pyproject)) {
        ...
        if (members) {
            for (const match of members[1].matchAll(/["']([^"']+)["']/g))
                globs.add(match[1]);
        }
    }
 ...
 */
function dotnetScopes(root) {
    const out = [];
    const describe = (projectFile) => {
        ...
        const runs = /<OutputType>\s*(Exe|WinExe)\s*<\/OutputType>/i.test(text) ||
    ...
    const scan = (dir, depth) => {
        ...
        for (const entry of entries) {
            if (entry.isDirectory()) {
                if (/^(node_modules|\.git|bin|obj|packages|dist|build)$/i.test(entry.name))
                    continue;
    ...
    if (solutions.length > 0) {
        for (const sln of solutions) {
            ...
            for (const match of readText(sln).matchAll(/^Project\("[^"]*"\)\s*=\s*"[^"]+",\s*"([^"]+)"/gim)) {
                ...
                if (!/\.(cs|fs|vb)proj$/i.test(projectPath))
                    continue;
                const scope = describe(path.resolve(slnDir, projectPath));
                if (scope && !out.some((s) => s.dir === scope.dir))
                    out.push(scope);
            }
        }
        return out;
    ...
    // signals reader uses.
    const projects = [];
    const scanProjects = (dir, depth) => {
        if (depth > 3 || projects.length > 100)
            return;
        let entries;
        try {
            entries = fs.readdirSync(dir, { withFileTypes: true });
...
function describeScope(root, dir) {
 ...
 */
function looksLikeApp(pkg, pyproject) {
    if (!pkg)
        return pyproject;
    const deps = {
        ...pkg.dependencies,
