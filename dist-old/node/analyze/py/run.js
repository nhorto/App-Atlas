 ...
 */
export async function findInterpreter(root) {
    const candidates = [];
    let warning = null;
    const failures = [];
    ...
    for (const dir of ['.venv', 'venv', 'env']) {
        for (const rel of [path.join(dir, 'Scripts', 'python.exe'), path.join(dir, 'bin', 'python')]) {
            ...
            if (fs.existsSync(full))
                candidates.push({ command: full, args: [] });
        }
    }
    ...
    for (const candidate of candidates) {
        ...
        if (left < MIN_PROBE_MS)
            break;
        const outcome = await probeInterpreter(candidate.command, candidate.args, left);
        if (outcome.kind === 'ok')
            return { interpreter: { ...candidate, version: outcome.version }, warning, missing: null };
 ...
 */
export function interpreterProblem(failures) {
    return (failures.find((f) => f.reason === 'timed-out') ??
...
function record(failures, command, outcome) {
    if (outcome.kind === 'timed-out')
        failures.push({ reason: 'timed-out', command, waitedMs: outcome.waitedMs });
    else if (outcome.kind === 'too-old')
        failures.push({ reason: 'too-old', command, version: outcome.version });
}
/**
 ...
 */
export async function probeInterpreter(command, args, timeoutMs = probeTimeoutMs()) {
    const result = await run(command, [...args, '-c', 'import sys;print("%d.%d" % sys.version_info[:2])'], '', timeoutMs);
    if (result.timedOut)
        return { kind: 'timed-out', waitedMs: timeoutMs };
    ...
    if (result.startError === 'ENOENT')
        return { kind: 'absent' };
    if (result.startError || !result.ok)
        return { kind: 'not-python' };
    ...
    if (major < 3 || (major === 3 && minor < MINIMUM_MINOR))
 ...
 */
export function unreadReason(missing) {
 ...
 */
export function missingInterpreterWarning(missing, fileCount) {
    const files = `${fileCount} Python ${fileCount === 1 ? 'file' : 'files'}`;
    const cost = fileCount === 1
    ...
    switch (missing.reason) {
        case 'timed-out':
            return (`Found ${files} but the Python interpreter did not answer in time: ${missing.command} was asked its ` +
        ...
        case 'too-old':
            return (`Found ${files} but the only Python here is ${missing.version} (${missing.command}), and App Atlas ` +
...
export function run(command, args, input, timeoutMs) {
    return new Promise((resolve) => {
        ...
        child.on('error', (err) => finish({ ok: false, stdout: '', stderr: err.message, timedOut: false, startError: codeOf(err) }));
