...
export async function analyzePython(ctx) {
    ...
    for (const ref of stale) {
        ...
        if (!file || !file.ok) {
            ...
            if (file?.error)
                warnings.push(`Could not read ${ref.relPath}: ${file.error}`);
    ...
    for (const ref of stale) {
        ...
        if (options.detectBoundaries) {
            ...
            bucket.boundaries.push(...detectPythonBoundaries({
                file,
                fileId,
...
async function extract(interpreter, batch, warnings) {
    const request = JSON.stringify({
        files: batch.map((ref) => ({ rel: ref.relPath, abs: ref.absPath })),
    });
    const result = await run(interpreter.command, [...interpreter.args, extractorPath()], request, EXTRACT_TIMEOUT_MS);
    if (!result.ok) {
    ...
    try {
        ...
        if (payload.error)
            warnings.push(`The Python reader reported: ${payload.error}`);
        return payload.files ?? [];
    }
    catch (err) {
        warnings.push(`The Python reader answered in a shape we could not read: ${err.message}`);
        return [];
    }
}
// ---------------------------------------------------------------------------
// Nodes
// ---------------------------------------------------------------------------
function buildNodes(ref, file, text) {
    const fileId = makeFileId(ref.relPath);
 ...
 * the cache does.
 */
function darkCellWarning(relPath, file) {
    ...
    if (dark.length > shown.length)
        shown.push(`and ${dark.length - shown.length} more`);
    return (`${relPath}: ${dark.length} of ${code} code cells are not Python, so nothing they declare is on the map — ` +
        shown.join(', '));
}
/** Which notebook cell a line fell in, or null for an ordinary file. */
function cellAt(file, line) {
...
function functionNode(id, parentId, ref, def, lines, cell, ownerName) {
...
function classNode(id, fileId, ref, def, lines) {
 ...
 */
function shallowFileNode(ref, root, because) {
 ...
 */
function linkFile(ref, file, index, declarations, edges) {
    const fileId = makeFileId(ref.relPath);
    ...
    for (const imp of file.imports ?? []) {
        ...
        for (const [exportedName, localName] of imp.names) {
            ...
            if (found)
                imported.set(localName, found);
        }
    }
    const own = declarations.get(ref.relPath) ?? new Map();
    const reference = (fromId, names) => {
