    ...
    async explain(nodeId, graph) {
        ...
        try {
            ...
            if (hit) {
                ...
                if (!grounded)
                    return { error: 'The cached description said something your routes disagree with, so it was dropped.' };
                apply(node, grounded);
            ...
            if (!text)
                return { error: `${backend.label} did not return a usable description.` };
            ...
            store?.writeExplanations(new Map([
                [key, { nodeId, tier: 'symbol', hash, text, backend: backend.id, createdAt: new Date().toISOString() }],
            ]));
