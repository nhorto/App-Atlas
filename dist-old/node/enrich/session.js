 ...
 */
export async function writeTheWords(options) {
    ...
    try {
        ...
        if (selection.backend.billing !== 'metered' && !options.neverAsk) {
            say(pc.dim(`  Writing ${cached.pendingItems} descriptions with ${selection.backend.label}` +
        ...
        say(pc.yellow(`  ! Explanations were skipped: ${err.message}`));
        return null;
    }
    finally {
        store.close();
 ...
 */
async function askPermission(estimate, options) {
    const { backend } = estimate;
    if (options.assumeYes)
        return true;
    if (options.neverAsk) {
        if (!options.quiet) {
            console.log(pc.dim(`  ${estimate.items} things have no description yet — run app-atlas analyze to write them.`));
        }
        return false;
    }
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
        ...
        if (!options.quiet) {
            ...
            console.log(pc.dim(estimate.costUsd === null
 ...
 */
function money(costUsd) {
    if (costUsd === null)
        return 'an amount we cannot estimate for this model';
    if (costUsd < 0.01)
        return 'well under a cent';
    if (costUsd < 1)
        return `roughly ${Math.ceil(costUsd * 100)}¢`;
 ...
 */
function misattributions(report) {
    if (report.misattributedRoutes.length === 0)
        return [];
    return [
...
export function describeRun(report) {
    if (report.declined)
        return ['  No explanations were written.'];
    ...
    if (report.backend === 'cache')
        return misattributions(report);
    const lines = [];
    const written = report.described + report.labelled;
    if (written > 0) {
        const cost = report.usage.costUsd && report.usage.costUsd > 0
    ...
    if (report.contradictions.length > 0) {
        ...
        lines.push(`  The description names ${report.contradictions.join(', ')}, which no detector found.`, '  Either the write-up over-reached or something real is missing from the map.');
    }
