...
export async function detectAgentClis(options = {}) {
    const found = [];
    for (const spec of CLIS) {
        if (await isInstalled(spec.command))
            found.push(makeBackend(spec, options));
    }
    return found;
}
export function agentCliById(id, options = {}) {
    const spec = CLIS.find((c) => c.id === id);
    return spec ? makeBackend(spec, options) : null;
}
function makeBackend(spec, options) {
    const backend = {
        id: spec.id,
        label: spec.label,
        ...
        probe: async () => {
            try {
                const reply = await invoke(spec, options, `Reply with exactly this word and nothing else: ${PROBE_TOKEN}`, PROBE_TIMEOUT_MS);
                ...
                if ((reply.usage?.costUsd ?? 0) > 0)
                    backend.billing = 'metered';
                if (reply.text.includes(PROBE_TOKEN))
                    return { ok: true };
                return { ok: false, reason: firstLine(reply.text) || 'gave an unexpected answer' };
            }
            catch (err) {
...
async function invoke(spec, options, prompt, timeoutMs, signal) {
    const outFile = spec.usesOutputFile
        ...
        : '';
    try {
        const { stdout, stderr, code } = await run(spec.command, spec.args(validModel(options.model), outFile), prompt, timeoutMs, signal);
        ...
        if (!text)
            throw new Error(firstLine(stderr) || `exited with code ${code}`);
        return { text };
    }
    finally {
        if (outFile)
...
function parseClaudeJson(stdout) {
    ...
    try {
        ...
        if (payload.is_error)
            return { error: payload.result ?? 'the CLI reported an error' };
        if (typeof payload.result !== 'string' || !payload.result.trim())
            return { error: 'returned nothing' };
        return {
            text: payload.result,
            usage: {
...
function run(command, args, stdin, timeoutMs, signal) {
...
function childEnv() {
    ...
    for (const key of Object.keys(env)) {
        if (key.startsWith('CLAUDE_CODE_') || key === 'CLAUDECODE' || key === 'CLAUDE_PID')
            delete env[key];
    }
...
function describeSpawnError(err, command) {
    if (err.code === 'ENOENT')
        return `${command} is not on your PATH`;
    return err.message;
}
