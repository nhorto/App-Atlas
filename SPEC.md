...
**M15 — five defects in one night, and the pattern under three of them: ✅ complete (2026-08-09).** Found by pointing 0.22.0 at seven public repositories in a single session — [#142](https://github.com/nhorto/App-Atlas/issues/142), [#143](https://github.com/nhorto/App-Atlas/issues/143), [#147](https://github.com/nhorto/App-Atlas/issues/147) (half), [#149](https://github.com/nhorto/App-Atlas/issues/149), [#152](https://github.com/nhorto/App-Atlas/issues/152). 22 new tests, 719 in total. `TOOL_VERSION` moves with the release: every one of these changes what a cached finding means.

**Seven repositories, seven defects.** lemmy, netbox, redash, documenso, ginreal, bruno, twenty. Not one came back clean, and the two largest — a 5,000-file NestJS server and a 16-package workspace — produced the worst two. The rate is not slowing, which is the honest measure of how far a fixture suite is from the shapes real code takes.

**Three tiers, three spellings, one gap — a route address the analyzer cannot resolve.** Flask **dropped** the door: `@routes.route(org_scoped_rule("/login"))` has no string literal, `if (!route) continue` threw away the handler, the methods and the guard, and redash reported 5 of its 28 routes with `/login`, `/forgot` and `/reset/<token>` among the missing (#142). Gin shows a **bare verb** — `"POST "`, `"GET /"` — because the group is a function parameter ([#151](https://github.com/nhorto/App-Atlas/issues/151)). NestJS **collides**: 25 of twenty's 46 controllers write their prefix as a template literal, so 141 route decorators collapse onto 44 doors, one of which carries sixty guards belonging to a dozen unrelated controllers ([#153](https://github.com/nhorto/App-Atlas/issues/153)). One root cause, and the failure modes have nothing in common — silence, nonsense, and a manufactured green. #142's answer is the pattern for the other two: keep the door, label it with the expression as written, leave `route` null so nothing downstream matches a prefix against it — plus a rule that two unresolved addresses must never merge on one key. **Whether that belongs in the merge rather than in three detectors is the open architectural question this milestone leaves.**
