...
- **A timestamp says how old the facts are; a commit says whether they are still true.** Those are different questions, and only the second is the one being asked. An atlas from last week describes a repo nobody committed to perfectly, and one from four minutes ago describes deleted code if an agent has been working — so the analyzer records the commit its findings describe (`meta.vcs`) and every answer compares it against the working tree in front of it. Both sides read `.git` rather than running `git`, and that is the design rather than an optimisation: the whole feature is a comparison of two commits resolved at different moments by different processes, and if one asked `rev-parse` while the other read files, a disagreement between the two resolvers would surface as a staleness warning about a repo that never moved. One implementation cannot disagree with itself; it also costs no subprocess per answer and behaves the same on a machine with no git. Where a commit cannot be established — no repository, an unreadable `.git`, an atlas written before this existed — the result is silence, and `stale` is *omitted* rather than sent as `false`: an absent field has to be handled, a `false` invites being trusted. A false alarm costs a re-analysis; a false all-clear costs an agent acting on a map of code that no longer exists. The check reads commits, so it says in the same breath that uncommitted edits are invisible to it, because a reader told the atlas matches HEAD would otherwise take that for a guarantee it matches the files on disk.
- **The tools are shaped around a task, not around an entity.** `what_calls` and `where_is` each took one name, which reads naturally and bills badly: an agent about to change five files has *one* question and was paying five round trips for it. Both now take a list, and the singular spelling still works because an agent that learned it a version ago is still right — passing both answers about everything named rather than silently letting one win. Three properties make a batch worth having over five calls, and each is a way it could have been done wrong. Every answer is labelled with the target it belongs to, because a run of unlabelled answers to different questions is worse than no batching: a reader that lines the second answer up against the first will act on it. `limit` is per target rather than shared, so one popular name cannot starve the next one. And a single target still returns byte-for-byte what it returned before, since most calls ask about one thing and nobody should pay for a feature they did not use. Twenty is the ceiling, and anything past it is dropped *and counted in the reply* — the same rule the rest of this surface follows, that a trimmed list which does not say so reads exactly like a complete one.
...
**M13 — what unfamiliar code said about the grammar tier: ✅ complete (2026-08-08).** Three defects from pointing the published package at public repositories nobody wrote a fixture for — [#129](https://github.com/nhorto/App-Atlas/issues/129), [#131](https://github.com/nhorto/App-Atlas/issues/131), [#132](https://github.com/nhorto/App-Atlas/issues/132). 8 new tests, 675 in total. `TOOL_VERSION` moves to 0.21.0: two of the three are the analyzer learning to read a shape, so a 0.20.0 cache holds an absence and a false accusation that were never true.

**Every tier's first contact with unfamiliar code produced a defect.** M12's four came from TypeScript and Python. Go gave one on the first repo it had not been written against; C# gave one on the first. That is now six findings across six repositories, against a fixture suite that was green throughout every one of them — which is not an argument that the fixtures are bad, but that a fixture is a hypothesis about what code looks like and only somebody else's repository can falsify it.

**A router kept on a struct field is still a router (#129).** [tailscale/setec](https://github.com/tailscale/setec) is a secrets server that registers eight `/api/*` routes, and App Atlas reported that it had none. `routerVars` tracked locals and typed parameters, because `mux := http.NewServeMux()` and `func serve(mux *http.ServeMux)` were the shapes the fixtures had; setec writes `cfg.Mux.HandleFunc(…)`, where the receiver is a field on a config struct. The type was already in `GDef.fields` — the grammar tier had read it and nothing asked. Zero routes to ten, and the report was the honest kind of wrong: it never said "no routes", it said nothing at all, which is worse on a service whose whole job is guarding secrets.

**Position, not scope, is what distinguishes two doors written side by side (#131).** On Microsoft's own eShopOnWeb the summary read `7 of 7 routes unprotected`; three of those seven are administrator-only and all three write data. The attribute is on the lambda — `app.MapPost("api/catalog-items", [Authorize(Roles = ADMINISTRATORS)] async (…) => …)` — and `attributesByScope` groups by enclosing *definition*, which here is one method holding both a guarded registration and a public one. Attaching by scope would have badged the public route as protected, so the fix keys on containment instead: `attributedInside` is the mirror of `chainedOnto`, and the five-line repro that pinned it has a `MapPost` and a `MapGet` that must come out different. 7 of 7 becomes 4 of 7.

**A hedge that fires when nothing is uncertain (#132).** [sharkdp/bat](https://github.com/sharkdp/bat) carries a syntax-*highlighting* test fixture full of ANSI escapes, and refusing to parse it is correct. Letting it soften *"no routes were found"* is not: the caveat means "a check may live in there", and a guard for a production route does not live in a highlighting fixture. This is [#116](https://github.com/nhorto/App-Atlas/issues/116) from the other side — a hedge nobody can act on teaches a reader to skip hedges, and the next one will be real. `unreadTestFiles` is subtracted where the sentence is built and nowhere else: the warning still names the file, and an unreadable *source* file still hedges exactly as #58 requires.

**The habit left litter, which is worth recording too.** A 1.6 MB `npm pack` tarball was committed by a `git add -A` in the middle of a verification run — the artifact rule producing artifacts. Untracked, and `*.tgz` added to `.gitignore`. A practice that runs constantly needs its by-products ignored by default, or it eventually commits one.

**M14 — two frameworks the map described from the wrong end: ✅ complete (2026-08-08).** Both found by pointing the published package at large public repositories in languages it claims to read — [#139](https://github.com/nhorto/App-Atlas/issues/139), [#140](https://github.com/nhorto/App-Atlas/issues/140). 15 new tests, 697 in total. `TOOL_VERSION` moves with the release: both change what a cached finding *means*, one by finding routes that were never there before and one by removing a thousand doors that should never have been.

**Django's route reader was unreachable code (#139).** [netbox-community/netbox](https://github.com/netbox-community/netbox) — 1,266 files, 377 `path()` calls — mapped as twelve ways in, of which ten were `manage.py`-style scripts and not one was HTTP. The reader was correct and had been for two releases; it simply sat at the bottom of `detectRoutes`, below the `if (!framework) return` that picks between FastAPI, Flask, Quart and Sanic. A Django `urls.py` imports `django.urls` and none of those four, so the function returned eighty lines above the code meant to handle it. Adding a single `import flask` to a Django routing table made every route appear, which is how the gate rather than the reader was identified — and is the whole argument for reproducing a mechanism instead of believing the first plausible story about a symptom.

**Every Python fixture in the suite imported FastAPI, which is why 682 green tests never noticed.** A fixture is a hypothesis about what code looks like; a suite made entirely of one framework's shapes cannot fail on another's absence. The new fixture imports Django and nothing else, and would go red the moment the gate came back.

**Finding the routes was the easy half.** With all 84 emitted, netbox read `84 of 84 routes have no auth check App Atlas can see` — a blanket false alarm about an application whose views sit behind a permission mixin, and a worse map than the silence it replaced. Django names a view in one file and defines it in another, and this reader does not follow that link, so the honest verdict is neither "open" nor "checked" but *not assessed*: **set aside, never hide** for the third time, after [#123](https://github.com/nhorto/App-Atlas/issues/123) and [#126](https://github.com/nhorto/App-Atlas/issues/126). The doors stay on the map, come out of the denominator rather than into the numerator, and the sentence says so. An unassessed door is not evidence of safety and not evidence of danger, and the two sentences the naive fix produced — every route open, then every route checked — were both the tool describing its own reach as the application's.

**`pub` in a Cargo workspace is not a public API (#140).** [LemmyNet/lemmy](https://github.com/LemmyNet/lemmy) reported **1,065 ways in**. Thirteen were real: container ports out of its compose files. The other 1,051 were 971 internal `pub` items across 361 files plus 80 helpers from a TypeScript test harness. Rust's module system *forces* `pub` on anything one crate needs another to see, so on a multi-crate workspace it describes nearly every internal item there is — and with no signal that the repo builds something you run, `classifyArchetype` fell through to `library` and `buildExportDoors` turned all of it into the front door. `exports.ts` predicts this in its own header: *"On an app, every helper a module exports would become a door and the picture would say nothing at all."*

**The third repeat of a fix `archetype.ts` already carries twice.** A WinUI desktop app was handed a public API of 154 of its own window classes; psf/requests was filed under "Something you run" on a leftover `__main__`. Both were settled by teaching the classifier one language's way of writing down "this builds an executable". Rust's is Cargo's own and needs no compiler — `src/main.rs` beside a manifest, or `[[bin]]` in one — and it is the exact counterpart of `<OutputType>Exe`. Lemmy goes from 1,065 doors to 14 and from "Code other code imports" to "Something you run", *because 3 crates build an executable*. That is a true and thin answer for a server with a hundred actix routes, and thin-and-true is the deal: reading actix routing is real work and inventing a route would be worse than omitting one.

**A stale cache proved its own warning.** Mid-verification, fastapi-template reported `11 of 23 routes unprotected` where a clean run gives 6 — a per-scope `.app-atlas` written before [#136](https://github.com/nhorto/App-Atlas/issues/136), reused because only the repo-root cache had been cleared. Nothing was wrong with the code; the cache was holding a verdict from an analyzer that could not yet read a `dependencies=[Depends(...)]`. That is exactly the failure the `TOOL_VERSION` comment describes, observed live, and the reason the number has to move with this release rather than at the next convenient moment.

**M15 — six defects in one night, and the pattern under three of them: ✅ complete (2026-08-09).** Found by pointing 0.22.0 at eight public repositories in a single session — [#142](https://github.com/nhorto/App-Atlas/issues/142), [#143](https://github.com/nhorto/App-Atlas/issues/143), [#147](https://github.com/nhorto/App-Atlas/issues/147) (half), [#149](https://github.com/nhorto/App-Atlas/issues/149), [#152](https://github.com/nhorto/App-Atlas/issues/152), [#153](https://github.com/nhorto/App-Atlas/issues/153). 26 new tests, 723 in total. `TOOL_VERSION` moves with the release: every one of these changes what a cached finding means.

**Eight repositories, eight defects.** lemmy, netbox, redash, documenso, ginreal, bruno, twenty, vercel/commerce. Not one came back clean, and the two largest — a 5,000-file NestJS server and a 16-package workspace — produced the worst two. The rate is not slowing, which is the honest measure of how far a fixture suite is from the shapes real code takes.

**Three tiers, three spellings, one gap — a route address the analyzer cannot resolve.** Flask **dropped** the door: `@routes.route(org_scoped_rule("/login"))` has no string literal, `if (!route) continue` threw away the handler, the methods and the guard, and redash reported 5 of its 28 routes with `/login`, `/forgot` and `/reset/<token>` among the missing (#142). Gin shows a **bare verb** — `"POST "`, `"GET /"` — because the group is a function parameter ([#151](https://github.com/nhorto/App-Atlas/issues/151)). NestJS **collided**: 25 of twenty's 46 controllers write their prefix as a template literal, so 141 route decorators collapsed onto 44 doors, one of which carried sixty guards belonging to a dozen unrelated controllers — and that, not the guard inversion, is what turned a 5,000-file server green (#153, fixed here: an argument that exists and cannot be read is now distinct from one that is absent, keys on its controller, and reports no address at all; 44 doors became 114 and no door carries more than five guards). One root cause, and the failure modes have nothing in common — silence, nonsense, and a manufactured green. #142's answer is the pattern the other two should share: keep the door, label it with the expression as written, leave `route` null so nothing downstream matches a prefix against it — plus a rule that two unresolved addresses must never merge on one key. Two of the three now follow it; Gin does not. **Whether that belongs in the merge rather than in three detectors is the open architectural question this milestone leaves.**

**The one direction this tool must never be wrong in, twice in one night.** Gin's `POST /login` read as protected — *certainly* — by a check named `UsersLogin`, because the login handler answers a wrong password with a 401 (#147). And twentyhq/twenty's OAuth callbacks read as protected by `PublicEndpointGuard`, a class whose docstring says it *"serves as documentation that the endpoint is intentionally accessible without authentication"* and whose body is `return true` (#152). Two frameworks, two spellings, the same inversion: evidence of openness counted as a lock.

**Confidence had been answering the wrong question.** `guardConfidence` says *does this check reach this route* — certain, when the check sits on the handler the router named. The Go detector's own grade says *is this a check at all* — `likely`, deliberately, because a function writing a 401 is "one function's behaviour standing in for a decision made by a framework we did not run". The merge took the first and discarded the second, so weak evidence became certain by being pointed at the right route. It is now the weaker of the two, and a framework's own vocabulary — `[Authorize]`, the decorators — is untouched.

**`declared-public`, because dropping the guard would have been the opposite error.** Nest has no `[AllowAnonymous]`; opting a route out of a global guard is written *as a guard that permits everything*. Simply not counting it would have moved twenty-seven deliberate decisions onto the worry list. So it joins the marketing page and the sign-in mount as "unchecked, with a reason" — the most explicit of the three, being one somebody wrote down. Read from the body and never the name: the fixture's `PublicApiKeyGuard` has *Public* in it and is a real check.

**Electron's IPC channels are doors, which the Rust tier had already decided (#149).** bruno's main process — 164 files, the privileged half of a desktop app — reported two ways in. It registers 255 `ipcMain.handle`/`.on` channels, the complete list of what the renderer may ask the host to do, filesystem and shell included. `#[tauri::command]`'s docstring made the whole argument two releases earlier: *"a map without them shows an engine nothing can reach."* Nothing new was needed to hold them — `endpointKind: 'ipc'` already existed, and Electron was already a known framework. They carry **no auth verdict**, exactly as Tauri commands do; 255 false alarms on one repo is the surest way to teach somebody to stop reading the auth line.

**A package with no source files reported fourteen ways in (#143).** documenso's `packages/assets` inherited the repo's compose ports, as did all sixteen scopes — fourteen facts presented 224 times. `repoRoot` was carrying two meanings, and splitting out `inheritDeploymentFiles` let the deployment question be answered separately from *is this the whole repo*. The ports were not dropped, which would have traded one wrong map for another: they are reported once, in the workspace summary, at the level they are true of.

**What the night says about the method.** Every fix but one was found by a controlled one-variable repro rather than by reading code: add `import flask` to a Django `urls.py` and watch every route appear; compare a literal path against a computed one in the same file; analyze one scope clean against cached. Four times a plausible mechanism died on contact with a repro — and once, in #152, a fix that looked right did **nothing at all**, because an imported name's symbol declares an `ImportSpecifier` rather than the class and the rule silently never fired. That one was caught by a three-file fixture, not by inspection. **Verify the mechanism fires; never infer that it must.**

**M15 addendum — the review answered the milestone's open question, then the review found what the milestone missed (2026-08-09).** The remaining three issues closed on #153's pattern: Gin's caller-passed groups stop colliding (#151, the third spelling — discriminated by the parameter's *type*, because gomount passes its root mux as a parameter with complete addresses and "arrived as a parameter" alone would have unresolved correct doors), the flagship tier reads a hand-rolled 401 the way Go always has (#155, commerce's `6 of 11` → `5 of 11`), and a Go login handler is the sign-in door rather than a route guarding itself (#147, followed two reference hops to the mint because that is the distance realworld keeps). The merge-versus-three-detectors question is settled by outcome: three detectors, one shared pattern, one shared rule — an address that cannot be read is not an address, and two of them are never one door.

**A code review of the batch found the batch's own defects, before they shipped.** Reviewing the six unreleased fixes produced four more: the unread-prefix key collided same-named controllers across files (#159 — #153's false green through a smaller hole, `SessionGuard@certain` on a door it was never written on), the computed-route key collided identical text across blueprints (#160), the web tier's `OpenKind` had fallen three kinds behind so an unchecked door wore a badge reading **"checked"** (#161 — live on npm since 0.22.0, not only in the batch), and — found by #159's own regression fixture failing for a second reason — a guardless class emits no alias, making its guarded namesake look unique to the very rule built to distrust duplicated names (#162; the owner hop now resolves file-locally, which keeps the true lock while refusing the borrowed one). Every one was repro'd before it was filed, and the night's rule held at one remove: the fixture caught what inspection missed, twice.

**M16 — three new ecosystems in one afternoon, and the reader's own blind spots: ✅ complete (2026-08-09).** paperless-ngx, immich and huginn — eleven repositories dogfooded lifetime, eleven with defects. [#170](https://github.com/nhorto/App-Atlas/issues/170): a DRF `router.register` table is the whole API and produced zero doors; one door per registration is the under-claiming floor, on #153's ellipsis. [#171](https://github.com/nhorto/App-Atlas/issues/171): 469 Ruby files mapped as "18 files, 1 way in" with no hedge — the most misleading map yet, all counts true; discovery now counts unread backbone languages and the summary, archetype and headline all own the blindness at dominance threshold. [#172](https://github.com/nhorto/App-Atlas/issues/172): `APP_GUARD` global guards were invisible and immich read 269 of 270 unprotected — the largest false alarm to date, now all-covered-at-likely with a catch-all that also reaches unresolved addresses. [#173](https://github.com/nhorto/App-Atlas/issues/173): the generated-header rules were dead code — written, documented, called by nothing — and a 7,702-line oazapfts client's 646 exports were a package's "API"; the rules now run, in either word order. [#174](https://github.com/nhorto/App-Atlas/issues/174): a test suite's fixture helpers are not a public API; the manifest is the evidence, all three facts required.

**The pre-release review caught the batch's own defect, again ([#180](https://github.com/nhorto/App-Atlas/issues/180)).** Guard findings passed the test-zone filter — latent forever, live the moment #172's catch-all let a guard originate anywhere and reach everything: a spec mocking `APP_GUARD` would have locked the whole application. Verified to leak before the fix, filtered like the router family after it. Two releases, two batch-review catches; the practice is no longer optional.

**M17 — the desktop release, and the two halves of a boundary neither of which was whole: ✅ complete (2026-08-09).** [#195](https://github.com/nhorto/App-Atlas/issues/195): the first real Tauri app this tool ever met reported **no ways in**. Eleven `#[command]` functions, files parsing perfectly, functions all on the map — and the detector matched only the fully qualified `#[tauri::command]`, while Rust is normally written by importing the macro and using the short name. #139's failure exactly: a reader that was correct and unreachable, with a fixture that only ever wrote the one spelling. The import is the gate, so clap's `#[command]` stays off. [#192](https://github.com/nhorto/App-Atlas/issues/192): #149 read `ipcMain` and left the preload, so a capability handed straight to renderer content — `shell.openExternal`, `webUtils.getPathForFile`, both of which bruno hands over — was routed through no channel and therefore on no map. Forwarders to existing channels emit nothing, because a second entry for one wire is the inflation #149 was careful about.

**Also in M17, from the same afternoon's dogfooding:** [#184](https://github.com/nhorto/App-Atlas/issues/184) a `0%` documentation grade printed against a `0/0` denominator, one line under the sentence explaining nothing had been read; [#185](https://github.com/nhorto/App-Atlas/issues/185) nuxt's 43-scope switcher, 33 of them one-file test fixtures; [#186](https://github.com/nhorto/App-Atlas/issues/186) a `cookies.delete('jwt')` logout that was the *single* entry on sveltejs/realworld's worry list; [#190](https://github.com/nhorto/App-Atlas/issues/190) a Next.js route reported as "protected by POST", because the framework requires the handler be called that.

**[#194](https://github.com/nhorto/App-Atlas/issues/194) — an attempt that was built, measured and reverted, recorded rather than repeated.** Composing a caller-passed Gin group's prefix worked on the fixture and made ginreal *worse*: 25 invented addresses where honest ellipses had been, because real Gin passes an inline `v1.Group("/users")` expression the rule never matched — and because the alias silently disarmed #151's safety net, which is keyed on the parameter name. **The fixture passed the entire time.** An invented address is the one failure this tool exists not to have; the diagnosis is on the issue for whoever tries next.

**The pre-release review found the batch's own regression for the fourth release running.** #185 stopped *listing* test-fixture scopes and thereby stopped anyone *reaching* one: `--scope fixture-basic` began answering "No app called that in this workspace". A rule about what is worth listing had quietly become a rule about what somebody may look at. Four releases, four catches.

**M18 — the question `generatedAt` was never answering.** Every MCP result already carried when the analysis was run, and a timestamp cannot tell an agent the thing it needs: whether these facts are still true. Ten minutes is nothing on a repo nobody touched and three features on one an agent is working in. The analyzer now writes down the commit its findings describe and every answer compares that against the working tree in front of it — the same instinct as the provenance line, one unit further along, in the unit that can actually be compared. Read from `.git` on both sides rather than by running `git`, because a comparison of two commits resolved by two different resolvers can disagree with itself, and a staleness warning about a repo that never moved would teach somebody to stop reading the line. Every case where a commit cannot be established is silence, and `stale` is omitted rather than sent as `false`: this is the surface where an all-clear nobody checked is the expensive direction. Borrowed in shape from repowise, which stamps `indexed_commit` on its own MCP responses — the idea, not a line of the implementation, which is AGPL and was never opened.
**M18 — one question, one call.** `what_calls` and `where_is` were shaped around a single entity each, which is the shape most MCP tools take and the reason agents spend so many turns on them: an agent about to touch five files asked five times. Both now take a list. The interesting part was not the loop but the three ways a batch goes wrong — answers that are not labelled (a reader lines the second up against the first and acts on it), a `limit` shared across targets (one popular name starves the next), and a silently trimmed list (which reads exactly like a complete one, and is how somebody concludes the twenty-first file has no callers). All three are defended by tests rather than by intent. The singular spelling still answers, because an agent that learned it a version ago is not wrong. Borrowed in shape from repowise, whose tools take several targets per call for the same reason — the idea, not a line of the implementation, which is AGPL and was never opened.
